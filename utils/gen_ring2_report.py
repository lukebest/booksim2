...
from rg_ring2_base import CORE_OUTSTANDING, Ring2BaseParams, run_batch as run_base
...
def _use_cjk_font() -> None:
    """Pick a CJK-capable face so topology titles render."""
...
每核 outstanding {meta.get('core_outstanding', CORE_OUTSTANDING)}。
...
S3 不预约 hop，读请求受每核 {meta.get('core_outstanding', CORE_OUTSTANDING)} 条 outstanding 限制，HA 按已到请求调度响应。
...
allpairs 那档 <code>bound</code> 只有 47 的直接原因：100 个事务的资源计数太小，
而依赖链没被计入。</li>
...
{_table(["层", "S0 RR", "S1 AIMD", "S2 request-grant", "S3 push-on-pull",
    ...
    ["每核 outstanding 读", str(CORE_OUTSTANDING), str(CORE_OUTSTANDING),
     str(CORE_OUTSTANDING), str(CORE_OUTSTANDING), str(CORE_OUTSTANDING),
     str(CORE_OUTSTANDING), str(CORE_OUTSTANDING), str(CORE_OUTSTANDING)],
    ["有 credit 时 RR 上环", "有", "有", "—",
     f"有（读请求受 {CORE_OUTSTANDING}/核 outstanding 卡）", "有", "有", "有", "有"],
...
S3 = 读请求本身是 POP 调度信息：每核最多 {CORE_OUTSTANDING} 条 outstanding 读，HA 对已到请求 RR 后给响应，不预约 hop。
