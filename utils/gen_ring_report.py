...
def svg_ring(order, is_cycle, dead_nodes, dead_links, cell=22, sacrificed=()):
...
<p>由于本报告的故障都是 12x16 网格上小而局部的空洞/断链，
...
每节点弹出 (N-1)/2 &asymp; 95 个 flit @2/cy 远小于传输延迟）。决定 makespan 的是
...
<style>
 ...
 .card{border:1px solid #cbd5e1;border-radius:8px;padding:8px;width:230px}
 ...
 .kpi{display:inline-block;background:#eff6ff;border:1px solid #bfdbfe;
   border-radius:8px;padding:8px 14px;margin:6px 8px 6px 0;font-size:15px}
    ...
    parts.append(f"""<p>拓扑 12x16 mesh ({MX*MY} 个节点)，横向链路 H={H} cycle，纵向链路 V={V} cycle，
PE&harr;router ramp 延迟 {RAMP} cycle，每条有向链路 1 flit/cycle，消息大小 M=1 flit。
在 Hamilton 环上做 allgather，比较两种模式：</p>
<ul>
...
<div>
 ...
 <span class="kpi">golden 双向环 makespan = <b>{g_bi}</b> cycles</span>
    ...
    parts.append('<div class="card"><h4>Golden snake ring (cycle, N=192)</h4>'
    ...
    parts.append('<p class="legend">slowdown% = (故障环 makespan / 同模式 golden makespan - 1) &times; 100。'
                 '节点故障场景参与节点更少 (ring_len &lt; 192)，与 192 节点的 golden 对比仅反映完成时间变化。</p>')
    ...
    parts.append('<h2>6. 结论 (Notes)</h2><ul>'
                 '<li>所有恢复出的环均通过 Hamilton 校验，且每个节点恰好弹出 (N-1)&middot;M 个 flit，'
                 '无链路/ramp 带宽冲突 (eject_ok=True)。</li>'
                 '<li>链路故障与 2x2 节点空洞保持颜色平衡，单/双向环均可成环，性能基本不退化 (0~3.4%)。</li>'
                 '<li>1x1 与 3x3 节点空洞破坏二部平衡，无闭合环：单向不可行；双向在开放路径上完成，'
                 '但最远节点需走近乎整条路径，makespan 约翻倍。</li>'
                 '<li>对不平衡节点空洞牺牲 1 个邻近节点再平衡后，单向重新可行，'
                 '双向 makespan 从约 2&times; 降回接近 golden（仅 +0.8~9.3%）。</li>'
                 '<li>双向环（2 flit/cycle ramp + 双向）在所有可行场景下约为单向环的一半。</li>'
                 '</ul>')
