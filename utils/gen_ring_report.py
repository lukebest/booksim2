...
<div>
 ...
 <span class="kpi">golden hybrid vband bi = <b>{g_hyb}</b> cycles</span>
    ...
    parts.append('<h2>6. 结论 (Notes)</h2><ul>'
                 ...
                 '双向 makespan 反而更短 (&minus;22.2%)；hybrid vband 在存活带内恢复环并跳过死区，'
                 '劣化率 &minus;10.2%。</li>'
                 '<li>hybrid vband 的带内环采用与全局 Hamilton 环<b>相同的故障感知恢复</b>：'
                 'find_ring（成环 &rarr; 开放路径回退）+ find_ring_rebalanced（不平衡时牺牲邻近节点）。'
                 '当故障落在带脊列 x=8 时，带内闭合环结构上不可行（区域色数奇偶阻隔），'
                 '回退为带内开放路径 + 双向 allgather，此时 makespan 接近整条带路径长度，劣化较大 '
                 '（如 center 脊链路/节点故障 +63~70%）；非脊故障的带可正常成环，劣化 &le;10%。</li>'
