...
def render(out_path=OUT):
    ...
    cfg = {
        ...
        "schemes": {"border": sch_border},
        "schematic": {"border": schematic_svg()},
    }
    data = json.dumps(cfg, separators=(",", ":"))

    html = """<!DOCTYPE html>
<html lang="zh"><head><meta charset="utf-8">
<title>16×16 Border AllGather 逐步演示（4×8×8 环 + AFIFO, 267 cy）</title>
<style>
...
<header>
<h1 id="hdr_title">16×16 Border AllGather — cycle-by-cycle 演示</h1>
<p id="hdr_desc">16×16 拆 4×(8×8 Hamilton 环) + 跨界 AFIFO(depth≤5, 实测峰值 2) · H=4, V=6 · 双向环, down-ramp=2 ·
makespan=<b id="hdr_mk"></b> cy · 同 link 双占用尝试=<b id="hdr_cf"></b>（calendar 全部错开，无同 cycle 双发）</p>
</header>
<div class="layout">
<div>
<div class="panel">
...
function syncSchemeUi(){{
  ...
  if(rn) rn.textContent = '4 象限各一条 8×8 Hamilton comb 环；下方两环脊贴中心边界，跨界走短弧经 AFIFO。';
}}
syncSchemeUi();
...
function draw(k){
  ...
  const phaseLabel = k < S.t_lap_bi ? '本环双向半圈'
    : (k < S.makespan-10 ? '跨界短弧扩散' : '收尾 eject');
