...
def hybrid_compare_section():
    """局部 Hamilton 环 + 全局树广播 的 allgather，与 multi-tree / 纯 ring 对比。"""
    try:
        import sim_hybrid_allgather as hy
    except Exception as exc:  # pragma: no cover
        return f"<div class='card'><h2>Hybrid allgather</h2><p>unavailable: {html.escape(str(exc))}</p></div>"

    ref = hy.reference_makespans()
    uni = [r for r in hy.sweep("uni")]
    bi = [r for r in hy.sweep("bi")]

    def best(rows):
        feas = [r for r in rows if r.get("feasible")]
        return min(feas, key=lambda r: r["makespan"])

    bu, bb = best(uni), best(bi)

    out = ["<div class='card'><h2>Hybrid: 局部 Hamilton 环 + 全局树广播</h2>"]
    out.append(
        "<p><strong>方案</strong>：把 12×16 mesh 按行切成 B 个水平带（每带 R=16/B 行，"
        "n<sub>b</sub>=R×12 个节点）。"
        "<strong>① 局部环</strong>：每个带内部跑一个 Hamilton 环 allgather，各带并行（占用互不相交的链路/ramp），"
        "结束后每个节点拿到本带全部 n<sub>b</sub> 条消息。"
        "<strong>② 全局树</strong>：B 个带块沿纵向做一次树状广播（相邻带间隔 R 行，跳延迟 R×V），"
        "把各带块互换，最终每节点拿到全部 N=192 条消息。"
        "两阶段串行（带块必须等本带环 allgather 完成才能广播），makespan = makespan(①)+makespan(②)。</p>"
    )
    out.append(
        "<p>端点自洽：<code>B=1</code> 即单个全局环（=纯 Hamilton 环）；"
        "<code>B=16</code> 即每行一个局部环 + 纵向列树。两相加的 down-ramp 下泄量恒为 "
        "(N−1)=191/节点（局部 n<sub>b</sub>−1 + 全局 (B−1)·n<sub>b</sub>），"
        "故下界被拆到两个阶段，hybrid 需付两段延迟。</p>"
    )

    # scheme comparison bar chart (apples: ramp_bw=1 group vs ramp_bw=2 group)
    labels = ["multi-tree r1", "ring-uni r1", f"hybrid-uni B{bu['B']} r1",
              "ring-bi r2", f"hybrid-bi B{bb['B']} r2"]
    values = [ref["multitree"], ref["ring_uni"], bu["makespan"],
              ref["ring_bi"], bb["makespan"]]
    out.append(bar_chart("Allgather makespan @ M=1 (lower=better)", labels, values))

    out.append("<h3>三方案对比 (M=1)</h3><table>")
    out.append("<tr><th>方案</th><th>ramp 带宽</th><th>makespan</th><th>vs multi-tree</th><th>vs 纯 ring(同 ramp)</th></tr>")
    mt = ref["multitree"]
    rows_cmp = [
        ("dimensional multi-tree", 1, mt, ref["ring_uni"]),
        ("纯 Hamilton ring (uni)", 1, ref["ring_uni"], ref["ring_uni"]),
        (f"hybrid 局部环+全局树 (uni, B={bu['B']})", 1, bu["makespan"], ref["ring_uni"]),
        ("纯 Hamilton ring (bi)", 2, ref["ring_bi"], ref["ring_bi"]),
        (f"hybrid 局部环+全局树 (bi, B={bb['B']})", 2, bb["makespan"], ref["ring_bi"]),
    ]
    for name, rb, ms, ringref in rows_cmp:
        vmt = f"{ms/mt:.2f}×"
        vring = "—" if ms == ringref else f"−{(1-ms/ringref)*100:.0f}%"
        out.append(f"<tr><td>{html.escape(name)}</td><td>{rb}</td><td>{ms}</td>"
                   f"<td>{vmt}</td><td>{vring}</td></tr>")
    out.append("</table>")

    # B-sweep tables
    for mode, rows in (("uni (ramp 1 flit/cy)", uni), ("bi (ramp 2 flit/cy)", bi)):
        out.append(f"<h3>带数 B 扫描 — {html.escape(mode)}</h3><table>")
        out.append("<tr><th>B 带数</th><th>R 行/带</th><th>n_b 节点/带</th>"
                   "<th>① 局部环</th><th>② 全局树</th><th>总 makespan</th></tr>")
        for r in rows:
            if not r.get("feasible"):
                out.append(f"<tr><td>{r.get('B','?')}</td><td colspan='5'>"
                           f"{html.escape(r['reason'])}</td></tr>")
                continue
            out.append(f"<tr><td>{r['B']}</td><td>{r['R']}</td><td>{r['n_b']}</td>"
                       f"<td>{r['phaseA_local_ring']}</td><td>{r['phaseB_global_tree']}</td>"
                       f"<td><strong>{r['makespan']}</strong></td></tr>")
        out.append("</table>")

    out.append(
        "<h3>结论</h3><ul>"
        f"<li><strong>hybrid 远优于纯环</strong>：单向最优 B={bu['B']} → {bu['makespan']} cycle，"
        f"较纯环 {ref['ring_uni']} 提速 {ref['ring_uni']/bu['makespan']:.1f}×；"
        f"双向最优 B={bb['B']} → {bb['makespan']}，较纯环 {ref['ring_bi']} 提速 "
        f"{ref['ring_bi']/bb['makespan']:.1f}×。原因：把一个 191 跳的长环拆成 B 个短环并行，"
        "phaseA 延迟随 B 线性下降。</li>"
        f"<li><strong>但 multi-tree 仍最优</strong>（{mt}，ramp1）：同 ramp 对比，hybrid-uni 最优 "
        f"{bu['makespan']} = {bu['makespan']/mt:.2f}× multi-tree。差距来自 hybrid 的<strong>两阶段串行</strong>——"
        "局部环与全局树各自付一段延迟与下泄；而 multi-tree 把全部流量融进<strong>一张无冲突 calendar</strong>，"
        f"X-then-Y 在网内 fork，整体只付一次最坏 gather，恰好命中 down-ramp 下界 (N−1)=191（实测 {mt}）。</li>"
        "<li><strong>存在最优带数</strong>：B 太小 → phaseA（长局部环）主导；B 太大 → phaseB 的 "
        "(B−1)·n_b 全局下泄量 + 纵向跨带延迟主导；二者权衡给出谷底（uni 在 B=8、bi 在 B=16 附近）。</li>"
        "<li><strong>取舍</strong>：hybrid 实现简单（只需带内环 + 一棵纵向树，无需全局 calendar 预配置），"
        "鲁棒性好（一个带故障只需重排该带的短环）；代价是比 multi-tree 慢 ~1.5×。"
        "对追求极致 makespan 用 multi-tree；对追求实现/容错简单且可接受 ~1.5× 开销，hybrid 是有吸引力的折中。</li>"
        "</ul>"
    )
    out.append("</div>")
    return "\n".join(out)
