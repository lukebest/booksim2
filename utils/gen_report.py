...
<p>这两条直接否定了之前「makespan=205 / eff=1.0」那组数：那依赖角节点 <strong>50–87 flit 的 eject 队列</strong>
+ 输出端口的<strong>无界滞留</strong>，正是上面禁止的踩踏场景。下面给出满足约束后的真实结果。</p>
...
<h3>带宽硬下界（eject 端口）</h3>
<p>无论怎么调度，每个节点单条 down-ramp 必须吞 (N−1)M flit、每 cycle 1 个：
<code>makespan ≥ (N−1)M + ramp</code> = <strong>48</strong>(6×8) / <strong>192</strong>(12×16)（M=1）。
零 eject 缓冲意味着这 (N−1) 次 eject 必须落在 (N−1) 个<strong>互不相同</strong>的 cycle 上，
且每个都恰好在 flit 到达当 cycle。要做到这点，只能靠<strong>有界网内等待</strong>把到达节奏「抹平」到 ≤1/cycle。</p>
...
<p><strong>6×8（带缓冲乐观值 78，eject 带宽下界 48）</strong></p>
<table style="font-size:13px">
<tr><th>每跳网内等待上限 W</th><th>makespan</th><th>vs 乐观值</th></tr>
...
<p><strong>12×16（带缓冲乐观值 205，eject 带宽下界 192）</strong></p>
<table style="font-size:13px">
<tr><th>每跳网内等待上限 W</th><th>makespan</th><th>vs 乐观值</th></tr>
...
<svg width="520" height="230" viewBox="0 0 520 230" xmlns="http://www.w3.org/2000/svg" style="max-width:100%">
  ...
  <line x1="60" y1="176" x2="490" y2="176" stroke="#059669" stroke-width="1" stroke-dasharray="4,3"/>
  <text x="492" y="180" font-size="9" fill="#059669">带宽下界 192</text>
  <line x1="60" y1="172" x2="490" y2="172" stroke="#dc2626" stroke-width="1" stroke-dasharray="4,3"/>
  <text x="492" y="166" font-size="9" fill="#dc2626">带缓冲乐观 205</text>
...
<p style="font-size:12px;color:#64748b">注：曲线由贪心启发式产生（<code>sched_no_eject_buffer.py</code>），是<strong>可达上界</strong>而非证明下界；
W 较大时偶有非单调是贪心装填的副作用。带宽硬下界 192 与带缓冲乐观值 205 仅作参照。</p>

<h3>结论</h3>
<ul>
<li><strong>eject 缓冲可以严格为 0</strong>（到达即取走），调度仍<strong>构造即无冲突</strong>——所有 W 下均可行。</li>
<li>但<strong>去掉 eject 缓冲不是免费的</strong>：为避免下行端口排队，必须用有界网内等待抹平到达节奏，
代价是 makespan 上升。现实工作点（E=0 且 W=4–8）：6×8 ≈ 103–108（1.3–1.4×），12×16 ≈ 330–376（1.6–1.8×）。</li>
<li><strong>之前的 205 / eff=1.0 不是合法工作点</strong>：它依赖 50–87 flit 的 eject 队列 + 端口无界滞留（踩踏），
应视为「缓冲充足时的乐观上界」，而非零缓冲、有界等待下的真实 makespan。</li>
<li><strong>硬下界</strong>来自 eject 端口带宽 <code>(N−1)M+ramp</code> = 192(12×16)/48(6×8)；
即便允许无界网内等待（仍 0 eject 缓冲），本贪心也只到 274/82，离下界仍有差距。</li>
<li>相关脚本：<code>utils/sched_no_eject_buffer.py</code>（E=0 + 有界 W 主结果）、
<code>utils/sched_zero_buffer.py</code>（W=0 完全刚性）、<code>utils/sched_buffer_sweep.py</code>（允许 eject 排队的对照）。</li>
</ul>
</div>"""
