...
DECK = [
    ...
    ("section", dict(
        ...
        key="带宽线已基本关死；公平缺陷是真实的，但它不必用带宽去换。"
            "真正要拍板的不是数字，而是层次 —— 动得了事务层就选 S16，动不了就选 S22。")),
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="④ 两头能同时拿住，但要动事务层", accent=True, b=[
                "S16 把决策点放回 completer：Jain 0.92818 → 0.98466，"
                "max/min → 1.0001，带宽只让 0.39%，硬件 900 FF‑eq。",
                "代价是它改的是 **HA 何时给谁发 DBIDResp**，属事务层调度。"
                "只改环上仲裁的 S22 完全不碰这一层，但要贵 15 倍、多让 1.8 个点带宽。",
                "[[所以这是一道层次题，不是一道数字题。]]"])],
    ...
    ("process", dict(
        ...
        title="一步无条件先做，一步二选一，外加一份不建议清单",
        steps=[
            dict(t="第一步 · 0 FF‑eq · 无条件", b=[
                "**每核在飞上限收到 32。**零硬件、无流量先验：覆盖最坏 89 拍 RTT "
                "只需约 26 笔，再多就只是在已经饱和的环上排队。",
                "[[uniform 绕环重发从上千拍降到 5 拍；非均匀流量带宽 +33%。]]"]),
            dict(t="二选一 A · 900 FF‑eq · 需动事务层", accent=True, b=[
                "**S16 授权保留。**写侧 Jain 0.92818 → 0.98466、带宽只让 0.39%、"
                "非均匀流量拿到 R* 的 99.74% —— 四个方案里唯一两头都不亏的。",
                "**前提**：completer 的授权调度可改。线格式与协议状态机不动，"
                "但要改 HA 何时给谁发 DBIDResp，须 IP 可改并过一次合规复核。"]),
            dict(t="二选一 B · 13,920 FF‑eq · 不碰事务层", b=[
                "**S22-stock。**当 HA 是不可改的硬 IP 时的落点：只改环上仲裁 —— "
                "这一拍谁让出哪个位置。协议、事务层、线格式零改动。",
                "Jain 0.94799、带宽 −2.21%，全面优于 S1 且便宜 34%。"
                "[[代价是比 A 贵 15 倍、多让 1.8 个点带宽。]]"]),
            dict(t="不建议", b=[
                "**S1 现状**：−16.3% 带宽换 +0.029 Jain，62 组参数扫描无解。",
                "**S19 / S20**：中价位，但在非均匀流量下控制回路一次都没触发。",
                "**S16 读侧**：S0 读的十核带宽只差 0.36%，本来就没有问题 —— "
                "不值得为 +0.47% 带宽再动一次事务层。"])],
        band="第 1 步可以立刻单独上线。第 2、3 步二选一，判据不是数字，而是一个工程事实："
             "completer 的授权调度能不能改 —— 能改选 A，不能改选 B。")),

    ("closing", dict(
        title=["汇报完毕", "请架构组决策"],
        lead=["需要拍板的一件事：HA / 内存控制器的授权调度能不能改？",
              "能改 → S16，900 FF‑eq、Jain 0.98466、带宽 −0.39%；"
              "不能改 → S22，13,920 FF‑eq、Jain 0.94799、带宽 −2.21%。"])),
]
