...
DECK = [
    ...
    ("media", dict(
        ...
        cards=[
            ...
            dict(t="空转为什么消不掉",
                 b=["在环绝对优先：注入只能挤 transit 留下的空隙，"
                    "而空隙的**分布**不由总量决定。加深队列、加宽端口都不改这一条。",
                    "[[结论：带宽目标应当是「不弄丢」，不是「抬上去」。]]"])])),
        ...
        blocks=[
            dict(kind="card", t="横轴的 FF‑eq 是什么", b=[
                ...
                "得到一个可以横向比较的面积代价。S1 = 21,220 FF‑eq 就是这么算出来的。"],
                 wt=1.02),
            dict(kind="card", t="前沿只剩两个点", b=[
                "S0（0 FF‑eq，η 0.8894）→ [[S16]]（900 FF‑eq，η **0.9395**）。"
                "900 FF‑eq 买到离理想控制器最近的位置，"
                "**比 S1 便宜 24 倍、η 高 0.174**。",
                "S22-stock（13,920，0.8883）、S19 / S20（5,840，0.8992）、"
                "S22 深队列（1,198,560，0.9210）全部被 S16 支配或性价比远低。"],
                 wt=1.34),
            dict(kind="band",
    ...
    ("section", dict(
        ...
        lead="两个方案，各两页：机制示意 + 与 S0 / S1 的实测对比。",
        key_label="SELECTION RULE",
        key="入选标准：在可实现 Pareto 前沿上或紧邻前沿，且目标值全部来自测量、"
    ...
    ("figside", dict(
        ...
        caption="左 = 写路径：把协议本来就要发的 DBIDResp 当授权用；"
                "右 = 读路径：同一套状态，改的是 CompData 的发出顺序。",
        ...
        blocks=[
            dict(kind="card", t="四段机制（与 S1 同格式）", b=[
                ...
                "**④ 执行**：在排队者里把授权给**迄今被服务最少**的那个核。"],
                 wt=1.68),
            dict(kind="card", accent=True, t="overcommit 跟着在飞上限走", b=[
                "在飞上限 128 时该取 64；收到 32 之后十个核总共只递上约 40 笔，"
                "64 永远扣不住任何东西，S16 会退化成 S0。重扫后取 **16**（上限的一半），"
                "12–16 是一段平台。"], wt=1.00),
            dict(kind="band",
                 text="扣授权不会制造气泡：占优核只是暂时没数据要发，"
                      "这一拍的 slot 谁能用谁拿走。", wt=0.62)])),

    ("media", dict(
        chrome="S16 实测：写侧与读侧", kicker="MEASURED · UNIFORM",
        stat="0.98466",
        ...
        cards=[
            ...
            dict(t="读：带宽还是正的",
                 b=["S16-R 拿到 R* 的 **98.41%**，比 S0 还高 0.47%，Jain 0.99449。"
                    "把控制放在请求端的 S1-R 则掉 **20.5%** 带宽、Jain 反而更低"
                    "（0.97474）—— 读侧管错了对象。"]),
            dict(t="读侧本来就不太不公平",
                 b=["S0 读的 Jain 已有 0.99067、max/min 1.0035。因为读的大流量是 HA "
                    "发出的 CompData，注入点在 HA 而不是核，"
                    "十个核不争同一个入环口。S16 的读侧价值主要在带宽与鲁棒性。"])])),
        ...
        cards=[
            ...
            dict(t="但被 S16 支配",
                 b=["S16 用 **1/15 的硬件**拿到更高的 Jain（0.98466）和更小的带宽代价"
                    "（0.39%）。S22 的价值在于它是**纯环上仲裁**，"
                    "不依赖 completer 侧的授权语义。"]),
            dict(t="不要买深队列变体",
                 b=["把下环队列从 12/8 加到 32/32，η 只从 0.8883 涨到 0.9210，"
                    "硬件贵 **86 倍**（1,198,560 FF‑eq）—— 花在队列上不如花在决策上。"])])),
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="③ S1 的路子方向错了", b=[
                "它用「自己上环失败」当拥塞信号，在无缓存环上这多半是别人的 transit "
                "造成的 —— 等于惩罚受害者；执行器又是闸门，让出的槽被沿途吃掉。",
                "[[结果是 −16.3% 带宽只换来 +0.029 Jain。]]"]),
            dict(t="④ 两头可以同时拿住", accent=True, b=[
                "S16 把决策点放回 completer：Jain 0.92818 → 0.98466，"
                "max/min → 1.0001，带宽只让 0.39%，硬件 900 FF‑eq。",
                "[[读侧同样成立，且带宽还是正的（+0.47%）。]]"])],
        band="贯穿全篇的机制结论：在无缓存环上「把速率从快核转移给慢核」本身是有损的 —— "
             "让出的气泡只能向下游传递、沿途会被吃掉。"
             "所以执行器要么选让位（S22），要么干脆别产生气泡（S16 扣授权）。")),

    ("process", dict(
        chrome="建议", kicker="RECOMMENDATION",
        title="按成本递增的三步，外加一份不建议清单",
        steps=[
            dict(t="立刻做 · 0 FF‑eq", b=[
                "**每核在飞上限收到 32。**零硬件、无流量先验：覆盖最坏 89 拍 RTT "
                "只需约 26 笔，再多就只是在已经饱和的环上排队。",
                "[[uniform 绕环重发从上千拍降到 5 拍；非均匀流量带宽 +33%。]]"]),
            dict(t="主推 · 900 FF‑eq", accent=True, b=[
                "**投 S16 授权保留。**写侧 Jain 0.92818 → 0.98466、带宽 −0.39%；"
                "读侧带宽 +0.47%、Jain 0.99449；非均匀流量拿到 R* 的 99.74%。",
                "不加总线、不加表，只改 completer 何时发 DBIDResp / CompData。"
                "唯一要定的参数是 overcommit = 在飞上限的一半。"]),
            dict(t="备选 · 13,920 FF‑eq", b=[
                "**S22-stock**：复用 S1 那条 6 bit 总线，去掉乘法器和令牌桶换成加法树。"
                "Jain 0.94799、带宽 −2.21%，全面优于 S1 且更便宜。",
                "[[它不依赖 completer 的授权语义，可作为 S16 不适用时的纯环上方案。]]"]),
            dict(t="不建议", b=[
                "**S1 现状**：−16.3% 带宽换 +0.029 Jain，62 组参数扫描无解。",
                "**S22 深队列**：η 只多 0.033，硬件贵 86 倍。",
                "**S19 / S20**：中价位，但在非均匀流量下控制回路一次都没触发。"])],
        band="三步之间没有依赖：第 1 步可以单独上线，第 2、3 步可并行评估。"
             "第 2 步是本次汇报建议的落点。")),

    ("closing", dict(
        title=["汇报完毕", "请架构组决策"],
        lead=["需要拍板的一件事：是否投 900 FF‑eq 做 S16 授权保留。",
              "它让 completer 按「谁被服务得最少」来发授权，写读两侧都不亏带宽。"])),
]
