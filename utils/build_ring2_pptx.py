...
DECK = [
    ...
    ("triple", dict(
        ...
        band="η 的等值线 R = η·R*·(1 + CoV²) 是抛物线：在 CoV = 0 处斜率为 0，即「一点不均衡不要钱」，"
             "与恒定汇率 κ 不符 —— 理想控制器沿上界从 CoV 0 走到 0.306，η 反而从 1.000 升到最高 1.036。"
             "所以全篇改用与上界平行的 φ。")),
    ...
    ("matrix", dict(
        ...
        band="η 与 φ 的排序基本一致，但 φ 的差值可直接读成 flit/cycle，并按理想汇率拆成带宽项与均衡项；"
             "η 的差值没有这种分解。")),
    ...
    ("media", dict(
        ...
        cards=[
            dict(t="S16 · overcommit", accent=True,
                 b=["oc 16→12 沿等 φ 线走（φ 0.906→0.894，CoV 0.130→0.076）；"
            ...
            dict(t="S1 · band × cap",
                 b=["同一 band 内 cap 1.0→0.25 每段 ε 0.68–0.79；"
                    ...
                    "工程用法：给定目标带宽 R′，φ 给出该方案能到的最低 CoV，ε 给出离它多远。"])])),
