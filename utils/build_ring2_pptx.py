...
DECK = [
        ...
        cards=[
            ...
            dict(t="空转是什么组成的",
                 b=["逐拍归因、无残项：**I-tag 主动让位 49.1%**（槽被留给上游预约）、"
                    ...
                    "但 Jain 掉 0.029；加深下环队列、加大在飞上限都是净亏。]]"])])),
    ...
    ("figside", dict(
        ...
        caption="红线 = 二阶锥规划的精确解，与仿真轮次无关；"
                "蓝点 = 官方 K = 20000 的 S0（5.5412 flit/cycle，Jain 0.92818）。",
        kicker="FORMAL DERIVATION",
        blocks=[
            dict(kind="card", t="三个变量的含义", b=[
                "**λc**（决策变量）= 第 c 个核每拍发出多少笔事务。十个核 → 十个数。",
                "**a(r,c)** = 核 c 每发一笔事务，平均占用资源 r 多少拍。"
                "r 遍历每条（链路，方向，VC）。路由固定、握手固定，所以这是个常数表。",
                "**约束 Σc a(r,c)·λc ≤ 1** = 任何一条链路每拍最多搬 1 flit。",
                "**Jain(λ) ≥ J** ⟺ ‖λ‖₂ ≤ (Σλc)/√(J·10)：一个二阶锥约束，"
                "所以 R(J) = max Σλc 是**精确解**，不是估计。"],
                 wt=1.72),
            dict(kind="band",
                 text="① 完全等速率的上限 R* = 5.7143，无约束峰值 R_max = 6.4000 —— "
                      "公平的固有代价 −10.71%。", wt=0.88),
            dict(kind="band",
                 text="② 曲线在高 Jain 段陡然变贵：最后 0.01 的 Jain 单价是"
                      "前面一大段的 3.6 倍。", wt=0.80),
            dict(kind="band",
                 text="③ S0 实测落在曲线**下方**：它既不最快也不最公平，"
                      "本身就不在前沿上。", wt=0.80)])),
    ...
    ("figside", dict(
        ...
        blocks=[
            ...
            dict(kind="card", t="严格前沿只剩两个点", b=[
                ...
                "I-tag（3,120，η 0.9124）与 S19 / S20（5,840，η 0.8992）"
                "在成本—收益平面上被 S16 支配，但改动层次不同，仍是有意义的工程备选；"
                "本节补充它们的机制和正式 K = 20000 对比。"],
                 wt=1.34),
            dict(kind="band",
    ...
    ("figside", dict(
        ...
        blocks=[
            ...
            dict(kind="card", t="前沿只剩两点", b=[
                "在飞上限收到 32 之后，S0 本身就已经拿到 R* 的 **98.76%**。"
                "在此之上 [[S16]]（900 FF‑eq）到 **99.74%**，E-tag 绕环从 4,384 降到 **35**。",
    ...
    ("section", dict(
        ...
        lead="五个方案、三种控制层次：机制示意 + 与 S0 / S1 的同口径实测对比。",
        key_label="SELECTION RULE",
        key="S16 是严格成本—收益前沿；I-tag、S19、S20 与 S22 是改动层次不同的"
            "近前沿工程备选。五者都可实现、数据来自测量，且不含流量 pattern 先验。")),
    ...
    ("figside", dict(
        chrome="I-tag t_inj=2 / hold=2（3,120 FF‑eq） · 本地触发 / 仲裁让位",
        img="26-itag-diagram.png", fig_w=8.75,
        caption="上 = 连续两拍抢不到本地出向 hop 就举 I-tag；"
                "下 = 最近的上游 donor 只让一个 slot，空 slot 沿环到达请求者。",
        kicker="NEAR-FRONTIER · I-TAG MECHANISM",
        blocks=[
            dict(kind="card", t="四段机制", b=[
                "**① 检测**：同 plane / 方向 / VC 的注入队列非空，"
                "但连续 t_inj = 2 拍没上环。",
                "**② 传递**：举 I-tag，带出饥饿请求者的位置；"
                "向上游寻找最近一个会跨过其瓶颈 hop 的 donor。",
                "**③ 反馈**：donor 只在一拍内暂停，制造一个可追踪的空 slot。",
                "**④ 执行**：空 slot 到达请求者后立即被它使用；"
                "hold = 2 让标记两拍后自动失效。"], wt=1.65),
            dict(kind="card", accent=True, t="它改的是环上仲裁", b=[
                "不动 requester / completer 的事务状态，不增加广播总线。"
                "新增的是每方向 / VC 的饥饿计数与 slot 预约状态，"
                "硬件估算 **3,120 FF-eq**。"], wt=0.98),
            dict(kind="band",
                 text="核心边界：I-tag 只把「一个具体 slot」送给一个具体饥饿点；"
                      "它不估计全局公平度，也不保证长期速率完全拉齐。", wt=0.72)])),

    ("media", dict(
        chrome="I-tag 实测：均匀写", kicker="MEASURED · UNIFORM",
        stat="0.97495",
        stat_sub=["I-tag 的 100 拍 Jain（S0 = 0.92818）",
                  "总带宽 −2.71%，整窗 max/min = 1.2526"],
        img="27-itag-compare.png",
        caption="三幅依次比较总带宽、100 拍瞬时 Jain、整窗最快 / 最慢核带宽比；"
                "每幅内三根柱都是 S0 / S1 / I-tag。",
        cards=[
            dict(t="作用是局部补 slot", accent=True,
                 b=["t_inj=2 让 I-tag 及早发现连续抢不到 hop 的节点；"
                    "hold=2 又限制单次让位只持续两拍，避免把上游长期封住。"]),
            dict(t="三项指标一起看",
                 b=["总带宽 **5.3908**（比 S0 低 2.71%）；"
                    "Jain **0.97495**；max/min **1.2526**。"
                    "与 S0 相比，两项公平性都明显改善；"
                    "与 S1 相比，Jain 更高、长期速率差更小，带宽也更高。"]),
            dict(t="适用边界",
                 b=["适合**只允许改环上仲裁、又不希望新增总线**的实现条件。"
                    "若目标是把长期速率差压到接近 1，仍需更强的跨核进度信息。"])])),
    ...
    ("figside", dict(
        ...
        kicker="NEAR-FRONTIER · WINDOW CC",
        blocks=[
        ...
        cards=[
            ...
            dict(t="当前工作点结果接近",
                 b=["**S19**：带宽 5.5255（vs S0 −0.28%）、Jain 0.93324、"
                    ...
                    "max/min 1.5514。"]),
            dict(t="工程选择",
                 b=["已经有 RTT 计时器时，S19 集成最轻；"
                    "已经有 tracker 水位与可用标记位时，S20 的拥塞含义更明确。"
                    "两者都涉及 requester 的事务发起窗口。"])])),
    ...
    ("section", dict(
        ...
        key="带宽线已基本关死；公平缺陷是真实的，但它不必用带宽去换。"
            "真正要拍板的不是单一排名，而是哪一层能改：completer、requester，"
            "还是只允许动环上仲裁。")),
    ...
    ("matrix", dict(
        ...
        cells=[
            dict(t="① 带宽线已基本关死", b=[
                "S0 = 96.97% R*，最忙的链路 96.98% 满；3.03% 的缺口已逐拍闭合"
                "（5 拍绕环 + 2,181 拍空转）。",
                "[[拥塞控制的带宽目标应是「不弄丢」，不是「抬上去」。]]"]),
            dict(t="② 公平缺陷是真实的", b=[
                "不是采样噪声：把抖动完全抹平后的 Jain 上限只有 0.96458，"
                "六快四慢是几何决定的固定分组（核间速率比 1.5673）。",
                "[[要提高均衡度就必须真的搬速率，只整时机不够。]]"]),
            dict(t="③ 严格前沿与工程备选要分开看", b=[
                "按「带宽 × 公平性 / 硬件」计算，严格前沿是 S16。"
                "I-tag、S19 / S20、S22 虽不在严格前沿上，但分别落在环上本地仲裁、"
                "requester 动态窗口和跨节点进度仲裁三个不同改动层。",
                "[[被数值支配，不等于在工程约束下没有选择价值。]]"]),
            dict(t="④ 两头能同时拿住，但层次决定落点", accent=True, b=[
                "S16 把决策点放回 completer：Jain 0.92818 → 0.98466，"
                "max/min → 1.0001，带宽只让 0.39%，硬件 900 FF‑eq。",
                "它需要改 HA 的授权调度。若该层不可改，可退到环上本地 I-tag、"
                "requester 窗口 S19 / S20，或只改环上仲裁的 S22。",
                "[[最终选择首先由「哪一层能改」决定，其次才是数字。]]"])],
        band="统一结论：不要把所有机制压成一个排名。S16、I-tag、S19 / S20、S22 "
             "分别交换 completer、环上本地、requester、跨节点仲裁的改动权限；"
             "先确定可改边界，再在对应层内选性能最好的方案。")),

    ("process", dict(
        chrome="建议", kicker="RECOMMENDATION",
        title="先收在飞，再按可改层次选择",
        steps=[
            dict(t="第一步 · 0 FF‑eq · 无条件", b=[
                "**每核在飞上限收到 32。**零硬件、无流量先验：覆盖最坏 89 拍 RTT "
                "只需约 26 笔，再多就只是在已经饱和的环上排队。",
                "[[uniform 绕环重发从上千拍降到 5 拍；非均匀流量带宽 +33%。]]"]),
            dict(t="completer 可改 · 900 FF‑eq", accent=True, b=[
                "**S16 授权保留。**写侧 Jain 0.92818 → 0.98466、带宽只让 0.39%、"
                "非均匀流量拿到 R* 的 99.74%。",
                "**前提**：completer 的授权调度可改。线格式与协议状态机不动，"
                "但要改 HA 何时给谁发 DBIDResp，须 IP 可改并过一次合规复核。"]),
            dict(t="只改环上仲裁 · 3,120 / 13,920 FF‑eq", b=[
                "**I-tag**：不加总线，只用本地饥饿检测和单 slot 预约；适合改动最小的场景。",
                "**S22**：复用 6 bit 进度总线，能直接比较谁领先、谁落后；"
                "适合允许跨节点进度信息、但事务层不可改的场景。"]),
            dict(t="requester 窗口可改 · 5,840 FF‑eq", b=[
                "**S19 / S20**：都用每核动态 outstanding 窗口。"
                "已有 RTT 计时器时选 S19；已有 tracker 水位和 1 bit 标记位时选 S20。",
                "两者不需要流量 pattern 先验，可作为 source-side 集成路径。"
                "读侧维持 S0，不另加控制。"])],
        band="建议顺序：第 1 步直接做；随后先问 completer 能不能改。"
             "能改优先验证 S16；不能改，再按现有硬件基础在 I-tag、S22、S19 / S20 中选。")),

    ("closing", dict(
        title=["汇报完毕", "请架构组决策"],
        lead=["第一问仍是：HA / 内存控制器的授权调度能不能改？",
              "能改优先 S16；不能改，再按可用基础选择本地 I-tag、"
              "跨节点 S22，或 requester 动态窗口 S19 / S20。"])),
]
