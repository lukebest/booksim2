...
DECK = [
    ...
    ("section", dict(
        ...
        key_label="KEY MESSAGE",
        key="S1 默认档：Jain 0.92818 → 0.95675（+0.029），总写带宽 −16.3%；"
            "S1T 把带宽收回后，Jain 回到与 S0 同一水平。"
    ...
    ("media", dict(
        ...
        stat="−16.3%", stat_sub=["S1 默认档相对 S0 的总写带宽", "Jain +0.029"],
        ...
        cards=[
            ...
            dict(t="S1T 每向预算（调参后）", accent=True,
                 b=["62 组 AIMD 参数网格选出的最优点（dir_split、cap 0.5、w 64、"
                    "burst 1）：带宽回到 **5.5426（+0.03%）**，Jain **0.92596**，"
                    "max/min 1.5815。"]),
            dict(t="两档各取一端",
                 b=["从 S1 走到 S1T：带宽 +19.5%，Jain −0.031。"
                    "62 组扫描里，带宽接近 S0 的配置，Jain 也回到 S0 附近。"])])),
    ...
    ("media", dict(
        ...
        cards=[
            ...
            dict(t="实测：两档各与一端逐拍重合", accent=True,
                 b=["**S1U = S1**：总带宽 4.6397、100 拍 Jain 0.95675、完成时间 72,144–86,175。",
                    "**S1D = S0**：5.5412 / 0.92818 / 56,779–72,144。",
                    "四组曲线两两完全一致，不是近似。"]),
            dict(t="原因在信号量级",
    ...
    ("media", dict(
        ...
        cards=[
            ...
            dict(t="两个口径说的不是一件事",
                 b=["100 拍 Jain 看窗内瞬时份额，完成时间比看长期速率。"
                    "S1 把 Jain 提高 0.029，完成时间比从 1.271 到 1.194，"
        ...
        blocks=[
            ...
            dict(kind="card", accent=True, t="这一格的信号语义", b=[
                ...
                "落后与领先可以直接比较。"],
                 wt=1.44),
            dict(kind="band",
                 text="62 组 AIMD 网格：带宽接近 S0 时 Jain 也回到 S0 附近，"
    ...
    ("figside", dict(
        ...
        caption="同一 fabric、同一 K=20000、同一 100 拍分箱。三个数依次为"
                "总带宽 / 分箱 Jain / 长期 max-min。",
        kicker="MEASURED, NOT ARGUED",
        blocks=[
            dict(kind="card", t="路由与背压：结构性失效", b=[
                "**S26** 5.443 / 0.9169 / 1.569 —— 三条轴全输给 S0：环已接近饱和，"
                "反向同样满，绕远还多占 (n−h) 个 hop·拍。",
                "**S27** 3.512 / 0.8837 / 1.263 —— 带宽 −36.6%，均衡度还比 S0 差。"
                "无缓存环上「保护链路」等于让链路空转。"], wt=1.06),
            dict(kind="card", t="显式速率：同一类的两个极点", b=[
                "**S28**（RCP 反馈）5.480 / 0.9316 / 1.547 —— 带宽几乎不亏，"
                "公平也几乎不动："
                "不公平不在 hop 的份额分配，在每核自己上环口的仲裁，"
                "算出的 share 高于落后核能上环的速率。",
                "**S28S**（每 hop 静态等分）3.338 / **0.9890** / **1.022** —— "
                "全研究最高的均衡度，代价 40% 带宽。"], wt=1.12),
            dict(kind="card", accent=True, t="S29 与 S1 的三轴数字", b=[
                "S29：5.207 / 0.9607 / 1.332；S1：4.640 / 0.9568 / 1.359。"
        ...
        blocks=[
            dict(kind="card", t="先认四个量", b=[
                ...
                "**R(J)**：公平度 ≥ J 时的最高总带宽。"],
                 wt=1.70),
            dict(kind="band",
                 text="红线点：固定 J，枚举十个核的速率组合，链路每拍 ≤ 1 flit，"
                      "取总带宽最高的那个。J 从低到高逐点重算即得整条红线"
                      "（凸优化全局上界，不是对仿真点拟合）。", wt=1.10),
            dict(kind="band",
                 text="红线上 = 该公平度下理论最好；红线下 = 机制损失；"
                      "红线上方不可能。最右端 J=1 即 R* = 5.7143。", wt=0.90)])),

    ("figside", dict(
        chrome="标量指标推导：把上界在 CoV 坐标下拉直", img="42-metric-derivation.png",
        fig_w=8.15,
        caption="左：LP 上界的 79 个点换到 CoV = √((1−J)/J) 坐标后是一条直线 "
                "R = R* + κ·CoV（κ = 2.177，RMS 残差 0.011 flit/cycle，最大相对残差 0.43%）；"
                "右：13 个方案 1 − φ 的两项分解。",
        kicker="FRONTIER → ONE SCALAR",
        blocks=[
            dict(kind="card", t="先回答「总带宽 × 分箱 Jain」是什么", b=[
                "J = 1/(1+CoV²)，且 n·J = **等效均分核数** n_eff（S0 为 9.28 个核）。"
                "所以 R·J = 每核平均带宽 × n_eff：只把「相当于被均等服务的那部分核」的带宽算进去。",
                "缺点：等 η 线 R·J = 常数是曲线，与上界**不平行**。理想控制器沿自己的上界从 "
                "J = 1 走到 0.914，η 从 1.000 变到 1.022 —— η 惩罚不均衡的汇率不是理想汇率。"],
                 wt=1.55),
            dict(kind="card", accent=True, t="推导出的标量 φ", b=[
                "上界是直线 ⇒ 理想控制器把 1 单位 CoV 换成 **κ = 2.177** flit/cycle。"
                "定义 **Φ = R − κ·CoV**（等速率等效带宽），**φ = Φ / R***。",
                "上界任意一点 φ ≡ 1；等 φ 线与上界平行；"
                "**1 − φ = (R* − R)/R* + κ·CoV/R*** = 带宽缺口 + 按理想汇率折成带宽的不均衡缺口。"],
                 wt=1.35),
            dict(kind="band",
                 text="φ 用一个数同时刻画总带宽和均衡度，单位就是带宽："
                      "S16 0.918、S0 0.864、S1 0.731、S28S 0.544。", wt=0.55)])),

    ("matrix", dict(
        chrome="φ 怎么用：表征一种算法、预测不均衡度、比较方案",
        kicker="WHAT φ, Δφ AND φ-RATIOS MEAN",
        title="调参降带宽 → 用 φ 反推 Jain；两方案相减 → 同等公平下的带宽差",
        cells=[
            dict(t="① φ 表征一种算法", b=[
                "一个方案的 (R, J) 工作点给出 φ。调参时沿**等 φ 线**移动的算法，"
                "就是「以理想汇率把带宽换成公平」的算法。",
                "φ 不变、带宽降到 R′ 时：**CoV′ = (R′ − φ·R*)/κ，J′ = 1/(1+CoV′²)**。"
                "预测值是上界：实测 J 不会超过它。"]),
            dict(t="② 预测的验证", b=[
                "S16 oc 16→12：带宽 5.459→5.275，预测 J **0.998**，实测 0.9943。",
                "S29 slot 2→1：预测 **0.943**，实测 0.9457。",
                "S1 spec cap 1.0→0.5：预测 **0.958**，实测 0.9443。",
                "S1 spec→harsh：预测 1.0，实测 0.912 —— 过了拐点，见 ③。"]),
            dict(t="③ 拐点与交换效率 ε", b=[
                "每段旋钮的实测斜率 κ′ = ΔR/ΔCoV，**ε = κ/κ′** = 实际换到的公平占理想汇率的比例。",
                "S16 oc 48→16 ε = 9.3（几乎白拿）；S29 slot 1→2 0.82；S1 cap 段 0.68–0.79；"
                "S28 0.31–0.80。",
                "过拐点后 ΔCoV ≤ 0、ε ≤ 0：只掉带宽不换公平，φ 直接按 ΔR/R* 下降。"]),
            dict(t="④ 差值与比值的含义", accent=True, b=[
                "**Δφ·R*** = 同等均衡度下 A 比 B 多出的带宽（把两者的不均衡差按理想汇率折成带宽后相减）。"
                "S16 − S0 = **+0.055**（−0.004 带宽 + 0.058 公平）；"
                "S1 − S0 = **−0.133**（−0.158 带宽 + 0.025 公平）。",
                "**φ_A / φ_B** = 等速率等效带宽之比 = 同等公平下完成同一批工作的速度比："
                "S16/S0 = 1.063，S0/S1 = 1.182。",
                "**(1−φ) 之比** = 离理想控制器的距离之比：S16 走完 S0 → 理想 40% 的路。"])],
        band="η 与 φ 的排序基本一致，但 φ 的差值可直接读成 flit/cycle，并按理想汇率拆成带宽项与公平项；"
             "η 的差值没有这种分解。")),

    ("media", dict(
        chrome="旋钮实测轨迹：φ 预测的有效范围",
        kicker="KNOB SWEEPS",
        stat="上界 ↔ 下界",
        stat_sub=["等 φ 线 = 按理想汇率交换（上界）",
                  "等 CoV 线 = 均匀限流、只掉带宽（下界）"],
        img="43-metric-knobs.png",
        caption="四个方案各扫一个参数（K 见图题）；灰虚线 = 过锚点的等 φ 线，灰点线 = 等 CoV 线，"
                "红线 = LP 上界。轨迹越贴近等 φ 线，该参数越接近以理想汇率换公平。",
        cards=[
            dict(t="S16 · overcommit", accent=True,
                 b=["oc 16→12 沿等 φ 线走（φ 0.906→0.894，J 0.984→0.994）；"
                    "12 以下 CoV 钉在 0.07、只掉带宽。**拐点 = oc 12**。"]),
            dict(t="S29 / S28",
                 b=["S29 slot 1→2 ε = 0.82，slot ≥ 3 只掉带宽（拐点 slot 2）。",
                    "S28 九个 α·burst 点挤在 0.05 CoV 内，ε 0.31–0.80：这个执行器换不动公平。"]),
            dict(t="S1 · band × cap",
                 b=["同一 band 内 cap 1.0→0.25 每段 ε 0.68–0.79；"
                    "band 从 gentle 换到 spec、harsh 时带宽下台阶而 CoV 回升。",
                    "工程用法：给定目标带宽 R′，φ 给出该方案能到的最好 Jain，ε 给出离它多远。"])])),
    ...
    ("figside", dict(
        ...
        blocks=[
            dict(kind="card", t="严格前沿只剩两个点", b=[
                ...
                "S1 为 21,220 FF‑eq。S22-stock（13,920，η 0.8883）是环仲裁备选；"
                "S19 / S20（5,840，η 0.8992）只作 requester 窗口控制的对照。"],
                 wt=1.10),
            dict(kind="card", t="补齐的四类：一个都没进前沿", b=[
                "S26 路由 1,560 / η 0.8724；S27 背压 1,960 / **0.5319**；"
                "S29 调度 4,440 / 0.8659；S28S 12,680 / **0.5861**；"
                "S28 速率 21,480 / 0.8405。",
                "五个点的 η **全部低于 S0 的 0.8894** —— 不是被 S16 挤掉，"
                "是连「什么都不做」都没超过。前沿与补齐之前完全一致。",
                "其中 S29：4,440 FF‑eq，约为 S22-stock 的 1/3，"
                "η 只差 S22-stock 0.022（详见 S29 两页）。"], wt=1.52),
            dict(kind="band",
                 text="判据是「离理想控制器有多近」：S16 把 η 从 0.8894 推到 "
                      "0.9395，走完 S0 与理想之间 45% 的距离。", wt=0.62)])),
    ...
    ("figside", dict(
        ...
        blocks=[
            ...
            dict(kind="band",
                 text="完成时间比是长期速率比的另一种读法；它和 100 拍 Jain 不是同一件事，见下页。",
                 wt=0.45)])),

    ("media", dict(
        chrome="完成时间跨度 vs 瞬时均衡度",
        kicker="TWO READINGS",
        stat="18 拍",
        ...
        caption="左：每个方案十核从最早到最晚完成的区间（千拍），右端数字 = 最晚 / 最早；"
                "右：100 拍 Jain 与完成时间比的散点。",
        cards=[
            dict(t="两个口径给出不同排序",
                 b=["S27 Jain 最低（0.884）但完成时间比 1.153，好于 S1 的 1.194：背压把所有核一起拖慢，"
                    "长期份额反而接近。S22 Jain 0.948 低于 S1 的 0.957，完成时间比却是 1.117 对 1.194。"]),
            dict(t="只有 S16 两项同时到底", accent=True,
                 b=["S16：Jain **0.9847**、完成时间比 **1.000**、总带宽 −0.39%。"
                    "S28S 也做到 1.015，但用 40% 带宽换来（总时长 119.8k）。"]),
            dict(t="对架构评审的意义",
                 b=["瞬时 Jain 回答「任一 100 拍里有没有核被饿」，完成时间比回答"
                    "「一批工作最后是谁拖尾」；两者都要看，S1 的 0.029 Jain 增益对应完成时间比 −0.077。"])])),
    ...
    ("media", dict(
        ...
        cards=[
            dict(t="写：几乎白拿的公平性", accent=True,
                 b=["Jain 0.92818 → **0.98466**，整窗 max/min 1.5663 → **1.0001**，"
                    "总带宽 −0.39%。",
                    "对照 S1：Jain 0.95675，带宽 −16.3%。"]),
            ...
            dict(t="所以读侧建议不做",
                 b=["S16-R 把带宽推到 98.41%（+0.47%）、Jain 到 0.99449，"
                    "[[收益在噪声量级，不值一次事务层改动。]]",
                    "对照：S1-R 读侧带宽 −20.5%、Jain 0.97474。"])])),

    ("media", dict(
        chrome="读侧补充：CompData = 1 / 2 / 4 flit 时 S0 的均衡度",
        ...
        cards=[
            dict(t="1 flit：不均最大", accent=True,
                 b=["每核读带宽 0.427–0.673，max/min **1.578**，100 拍 Jain **0.9623**；"
                    "C0 / C10 最慢、C8 / C18 次之 —— 与写侧同一组慢核。总带宽 5.387（R* 的 94.3%）。"]),
            dict(t="2 flit 齐；4 flit 长期齐、瞬时抖",
                 b=["2 flit：max/min 1.0035、Jain 0.9907。",
                    "4 flit：max/min **1.0014**，但 Jain **0.9751**、最差箱 0.811 —— "
    ...
    ("media", dict(
        ...
        cards=[
            dict(t="与 S1 同口径", accent=True,
                 b=["S22：Jain 0.94799，带宽 −2.21%，max/min 1.2230，13,920 FF‑eq。",
                    "S1：Jain 0.95675，带宽 −16.3%，max/min 1.3586，21,220 FF‑eq。"]),
    ...
    ("media", dict(
        ...
        stat="0.96074",
        stat_sub=["S29 写侧 100 拍 Jain（S0 = 0.92818）", "总写带宽 −6.04%"],
        img="31-s29-compare.png",
        caption="五根柱依次 S0 / S1 / S22 / S16 / S29；橙点线 = S1 的水平，"
                "橙虚线 = 理论上限 R*。写侧 K = 20000，与前面各页同口径。",
        cards=[
            dict(t="与 S1 同口径", accent=True,
                 b=["S29：带宽 5.2067，Jain 0.96074，max/min 1.3323，4,440 FF‑eq。",
                    "S1：带宽 4.6397，Jain 0.95675，max/min 1.3586，21,220 FF‑eq。"]),
            dict(t="对 S22：便宜 3.1 倍，略差一点",
                 b=["Jain 0.96074 高于 S22 的 0.94799，但带宽 −6.04% 对 −2.21%、"
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="③ 上界与评价口径已建立", b=[
                ...
                "再用 100 拍 Jain、完成时间比、硬件 FF‑eq 和流量迁移统一评估。",
            ...
            dict(t="④ 控制点决定方案", accent=True, b=[
                ...
                "第 21 页空着的四格已实测：S26 / S27 结构性失效，S28 不动公平，"
                "S28S 用 40% 带宽换公平；S29 为 5.207 / 0.9607 / 1.332。",
    ...
    ("process", dict(
        ...
        steps=[
            ...
            dict(t="备选乙 · S29 日历让路", b=[
                ...
                "S22 的 −2.21%，Jain 0.96074。",
                "**定位**：环仲裁分支里硬件最便宜的一档。"]),
            dict(t="边界 · 明确不进入建议", b=[
                "**S19 / S20**：仅作 requester-side 对照；实测公平性几乎不变。",
                "**S26 / S27 / S28**：路由、背压、显式速率三类实测均劣于 S0，"
                "不再推进。",
                "**读侧**：128 B CompData 下 S0 已达 Jain 0.99067、max/min 1.0035，维持现状；"
