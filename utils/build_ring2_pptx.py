...
DECK = [
    ...
    ("section", dict(
        ...
        lead="按第 19 页的两个正交维度分类：先按拥塞信号（行），再按控制点（列），"
    ...
    ("matrix", dict(
        chrome="分类 · 拥塞信号（第 19 页的行）",
        kicker="TAXONOMY BY SIGNAL  =  PAGE 19 ROWS",
        title="听什么决定盲区（= 第 19 页的行）",
        cells=[
            ...
            dict(t="显式等级", accent=True, b=[
                "**代表**：S1 / S1T / S15 / S22。",
                "**听什么**：广播的拥塞等级或进度。",
                "**优**：全局可见。**缺**：分不清「我太贪」和「我被挤」；S22 只换了线上内容。"]),
            dict(t="显式速率", b=[
                "**代表**：S28 / S28S。",
                "**听什么**：瓶颈自己算出的 share。",
    ...
    ("matrix", dict(
        chrome="分类 · 控制点（第 19 页的列）",
        kicker="TAXONOMY BY CONTROL POINT  =  PAGE 19 COLS",
        title="动手处决定天花板（= 第 19 页的列）",
        cells=[
            dict(t="路径选择", b=[
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="④ 控制点决定方案", accent=True, b=[
                ...
                "第 19 页空着的四格已实测：S26 / S27 结构性失效，S28 不动公平，"
                "S28S 用 40% 带宽换公平，只有 S29 三条轴优于 S1。",
