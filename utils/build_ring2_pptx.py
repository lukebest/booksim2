...
DECK = [
    ...
    ("figside", dict(
        ...
        blocks=[
            ...
            dict(kind="card", t="严格前沿只剩两个点", b=[
                ...
                "S19 / S20（5,840，η 0.8992）与 S22-stock（13,920，η 0.8883）"
    ...
    ("section", dict(
        ...
        lead="四个方案、三种控制层次：机制示意 + 与 S0 / S1 的同口径实测对比。",
        key_label="SELECTION RULE",
        key="S16 是严格成本—收益前沿；S19、S20 与 S22 是改动层次不同的"
            "近前沿工程备选。四者都可实现、数据来自测量，且不含流量 pattern 先验。")),
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="③ 严格前沿与工程备选要分开看", b=[
                ...
                "S19 / S20、S22 虽不在严格前沿上，但分别落在 requester 动态窗口"
                "和跨节点进度仲裁两个不同改动层。",
            ...
            dict(t="④ 两头能同时拿住，但层次决定落点", accent=True, b=[
                ...
                "它需要改 HA 的授权调度。若该层不可改，可退到 requester 窗口"
                "S19 / S20，或只改环上仲裁的 S22。",
                "[[最终选择首先由「哪一层能改」决定，其次才是数字。]]"])],
        band="统一结论：不要把所有机制压成一个排名。S16、S19 / S20、S22 "
             "分别交换 completer、requester、跨节点仲裁的改动权限；"
    ...
    ("process", dict(
        ...
        band="建议顺序：第 1 步直接做；随后先问 completer 能不能改。"
             "能改优先验证 S16；不能改，再按现有硬件基础在 S22、S19 / S20 中选。")),

    ("closing", dict(
        title=["汇报完毕", "请架构组决策"],
        lead=["第一问仍是：HA / 内存控制器的授权调度能不能改？",
              "能改优先 S16；不能改，再选择跨节点 S22，"
              "或 requester 动态窗口 S19 / S20。"])),
]
