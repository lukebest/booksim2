...
DECK = [
    ...
    ("section", dict(
        ...
        lead="按「在哪里动手」和「听什么信号」把文献里的拥塞控制分成八类，"
             "给出各类优缺点，定位 S1，并补齐本研究原本没有代表方案的四类。",
        key_label="KEY MESSAGE",
        key="S1 的缺陷不是它自己的，是整一行共有的：触发降速的是节点自己的失败，"
            "而无缓存环上这些失败大多由别人造成。换控制点比换参数有用得多。")),

    ("matrix", dict(
        chrome="分类 · 控制点 ①–④", kicker="TAXONOMY BY CONTROL POINT · 1 / 2",
        title="动手的地方决定天花板：路由、背压、两种源端限速",
        cells=[
            dict(t="① 自适应路由", b=[
                "**代表**：UGAL / Valiant / CQR。",
                "**动手处**：路径选择。",
                "**优**：不减少任何人的注入量，只换路径，理论上零吞吐代价。",
                "**缺**：要有非最短路可走，而绕远本身要多占链路容量。"]),
            dict(t="② 逐跳背压", b=[
                "**代表**：Credit FC / PFC / wormhole 背压。",
                "**动手处**：上游链路。",
                "**优**：结构上不丢包，不需要测量任何全局量，实现最简单。",
                "**缺**：需要可暂停的队列；会形成拥塞树，伤及无辜。"]),
            dict(t="③ 源端限速 · 速率型", accent=True, b=[
                "**代表**：AIMD / DCQCN / TIMELY / QCN。",
                "**动手处**：注入令牌桶。",
                "**优**：直接控制注入总量，稳定性有成熟理论，执行器便宜。",
                "**缺**：闸门只能「扣住」，让出的槽会被沿途吃掉；量化亏带宽。"]),
            dict(t="④ 源端限速 · 窗口型", b=[
                "**代表**：TCP / DCTCP / Swift / outstanding cap。",
                "**动手处**：在飞事务数。",
                "**优**：与 CHI 事务模型天然对齐；有名额可突发，不做逐拍量化。",
                "**缺**：只管「有多少在飞」，不决定「瓶颈处先服务谁」。"])])),

    ("matrix", dict(
        chrome="分类 · 控制点 ⑤–⑧", kicker="TAXONOMY BY CONTROL POINT · 2 / 2",
        title="其余四类：把控制点从源端搬到网内、接收端或调度器",
        cells=[
            dict(t="⑤ 网内仲裁 / 公平队列", b=[
                "**代表**：WFQ / age-based arb / iSLIP / HiRD / I-tag。",
                "**动手处**：环上仲裁。",
                "**优**：控制点就在拥塞发生地，不需要端到端环路，不扣住流量。",
                "**缺**：只能改「这一拍谁先用」，要动仲裁器。"]),
            dict(t="⑥ 预约 / 调度式准入", b=[
                "**代表**：Fastpass / ExpressPass / TDMA / token ring。",
                "**动手处**：预分配时隙。",
                "**优**：拥塞不形成；最坏情况是调度性质，不是控制环路性质。",
                "**缺**：为空闲者保留的时隙直接浪费；帧长决定粒度。"]),
            dict(t="⑦ 接收端驱动", b=[
                "**代表**：Homa / NDP / pHost / ExpressPass。",
                "**动手处**：completer 授权。",
                "**优**：控制点在真正拥塞的下环侧；扣住授权不会造成气泡。",
                "**缺**：要动事务层；拥塞不在接收端时无效。"]),
            dict(t="⑧ 显式速率反馈", b=[
                "**代表**：XCP / RCP / HPCC。",
                "**动手处**：瓶颈自己算速率并回传。",
                "**优**：源端不必猜；同一 hop 上所有核拿到同一个数，天然公平。",
                "**缺**：反馈信息量最大，总线最宽；收敛要多窗。"])],
        band="第二个维度是「听什么信号」：本地观测 / 时延 / ECN / 显式等级 / 显式速率，"
             "与控制点正交 —— 同一执行器换个信号就是另一个方案"
             "（S17↔S18、S19↔S20）。")),

    ("figside", dict(
        chrome="S1 在分类中的位置", img="16a-cc-taxonomy.png", fig_w=8.30,
        caption="列 = 控制点（决定天花板）；行 = 拥塞信号（决定盲区）。"
                "灰底 = 本研究已有方案，红框 = S1 所在格，绿框 = 本次补齐的四类。",
        kicker="WHERE S1 SITS",
        blocks=[
            dict(kind="card", t="S1 = 源端限速 · 速率型 × 显式拥塞等级", b=[
                "**控制点**：每核每窗的整型注入预算，出口是令牌桶闸门 —— 第 ③ 类。",
                "**信号**：3 bit 拥塞等级，专用广播总线，固定 30 拍 —— 是「等级」"
                "而不是「速率」。同格还有 S1T 与 S15。"], wt=1.30),
            dict(kind="card", accent=True, t="缺陷属于「行」，不属于 S1 本身", b=[
                "乘性减由**自己上环失败**触发，而无缓存环上这些失败大多是别人的 "
                "transit 造成的 —— 于是惩罚受害者。凡是以「本节点观测到的失败 / 时延」"
                "为信号的格子都有同一个盲区。",
                "[[同一行里 S22 是唯一的例外：位宽完全不变，只把线上的内容从"
                "「我有多堵」换成「我这窗做了多少」，落后与领先才能直接比较。]]"],
                 wt=1.44),
            dict(kind="band",
                 text="所以调参救不了 S1：62 组 AIMD 网格没有一组两头都好，"
                      "因为它换的是列里的刻度，不是格子。", wt=0.74)])),
    ...
    ("media", dict(
        ...
        stat="4 / 8",
        stat_sub=["八类里原本没有代表方案的类数", "S26 / S27 / S28 / S29 已全部实现并实测"],
    ...
    ("figside", dict(
        ...
        blocks=[
            ...
            dict(kind="card", accent=True, t="S29 是四类里唯一三条轴全胜 S1 的", b=[
                "5.207 / 0.9607 / 1.332 对 S1 的 4.640 / 0.9568 / 1.359：带宽多 "
                "12.2%，两个公平指标都更好，总线 10 bit/16 拍 对 6 bit + 20 项表。",
                "原因不在「预约」而在**执行器**：复用 S22 的让位，让出的槽是指名的；"
                "令牌桶扣出的槽沿途谁都能吃。",
                "前沿仍是 S0 → S16（见 Pareto 页），但「闸门类要 16–40% 带宽换公平、"
                "让位类不用」已是跨四族的规律。"], wt=1.30)])),
    ...
    ("figside", dict(
        ...
        blocks=[
            dict(kind="card", t="严格前沿只剩两个点", b=[
                ...
                "S22-stock（13,920，η 0.8883）是环仲裁备选；"
            ...
            dict(kind="card", t="补齐的四类：一个都没进前沿", b=[
                ...
                "其中 S29 值得记住：4,440 FF‑eq 是 S22-stock 的 1/3、S1 的 1/5，"
    ...
    ("media", dict(
        ...
        cards=[
            dict(t="比 S1 三条轴全胜", accent=True,
                 b=["带宽 5.2067 vs 4.6397（**多 12.2%**）、"
                    "Jain 0.96074 vs 0.95675、max/min 1.3323 vs 1.3586。"
                    "硬件 4,440 vs **21,220** FF‑eq。"
                    "[[S1 那一格换成日历触发就全面更好。]]"]),
    ...
    ("matrix", dict(
        ...
        cells=[
            ...
            dict(t="④ 控制点决定方案", accent=True, b=[
                ...
                "八类分类里原本空着的四类已全部实现并实测：自适应路由（S26）与逐跳背压"
                "（S27）结构性失效，显式速率要么不动公平（S28）要么用 40% 带宽换公平"
                "（S28S），只有预约调度（S29，复用让位执行器）三条轴全面优于 S1。",
    ...
    ("process", dict(
        ...
        steps=[
            ...
            dict(t="备选乙 · S29 日历让路", b=[
                ...
                "**代价**：4,440 FF‑eq（S22 的 1/3、S1 的 1/5）；带宽 −6.04% 对 "
                "S22 的 −2.21%，Jain 0.96074 反而更高。",
                "**定位**：三条轴全面优于 S1，是环仲裁分支的最便宜落点。"]),
            dict(t="边界 · 明确不进入建议", b=[
                "**S19 / S20**：仅作 requester-side 对照；实测公平性几乎不变。",
                "**S26 / S27 / S28**：路由、背压、显式速率三类实测均劣于 S0，"
                "不再推进。",
                "**读侧**：S0 已达 Jain 0.99067、max/min 1.0035，维持现状。"])],
