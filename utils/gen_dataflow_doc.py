...
def allgather_description():
    return """<p><strong>最优方案不是 gather+broadcast。</strong>把数据汇经 root 再广播会串行化两阶段并制造 root 热点，
        ...
        <p><strong>零 eject 缓冲 + 无冲突 + non-blocking（有界每跳等待 W）可行，但不免费</strong>。
        用 CP-SAT 精确求解（<code>utils/sched_ilp.py</code>，OR-Tools）得到真实最优：</p>
        <table style="font-size:12px;border-collapse:collapse;width:100%;margin:8px 0">
        <tr style="background:#f1f5f9"><th>Mesh</th><th>带缓冲乐观</th><th>松弛 LB*</th><th>CP-SAT 真实最优(E=0)</th><th>贪心对照</th></tr>
        <tr><td>6×8</td><td>78</td><td>78</td><td><strong>81（已证, W≥4）</strong></td><td>103–108</td></tr>
        <tr style="background:#ecfdf5"><td>12×16</td><td>205</td><td>205</td><td><strong>[257,313]（W=8）</strong></td><td>330</td></tr>
        </table>
        ...
        <h3 style="font-size:15px;margin:16px 0 8px">buffer 深度 vs 最优 makespan（B=2, lat=4, CP-SAT）</h3>
        <p>固定 B=2、E=0，把 buffer 深度建模为每跳网内等待上限 W，用 CP-SAT 求每个 W 的最优 makespan：</p>
        <table style="font-size:12px;border-collapse:collapse;width:100%;margin:8px 0">
        <tr style="background:#f1f5f9"><th>buffer 深度 W</th><th>8×8 (lat=4) 证明最优</th><th>16×16 (lat=4) CP-SAT 可达</th></tr>
        <tr><td>0（网内无 buffer）</td><td>[78,90]</td><td>424</td></tr>
        <tr><td>1</td><td>[70,73]</td><td>—</td></tr>
        <tr style="background:#ecfdf5"><td>2</td><td><strong>67（已证最优）</strong></td><td>343</td></tr>
        <tr><td>8 / 16</td><td>67</td><td>315 / 301</td></tr>
        </table>
        <p style="font-size:12px;color:#475569"><strong>buffer 深度 2 即命中 8×8 全局最优 67，再加深无收益</strong>；W=0 代价最大（+34%）。
        16×16（256 节点）超出 CP-SAT 证明能力，只给区间。延迟地板 58(8×8)/122(16×16) 与 B 无关，深 buffer 无法突破。
        详见 <a href="report.html">report.html</a>「案例：B=2、16×16…」。</p>"""
...
<svg width="480" height="210" viewBox="0 0 480 210" xmlns="http://www.w3.org/2000/svg">
  ...
  <line x1="50" y1="162" x2="455" y2="162" stroke="#94a3b8" stroke-dasharray="2,3"/>
  <text x="457" y="156" font-size="8" fill="#94a3b8">松弛LB*205</text>
  <line x1="50" y1="150" x2="455" y2="150" stroke="#059669" stroke-dasharray="4,3"/>
  <text x="457" y="144" font-size="8" fill="#059669">CP-SAT≥257</text>
