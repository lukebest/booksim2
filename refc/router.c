                 ...
                 uint32_t calendar_external_base)
{
    ...
    combine_unit_init(&router->combine);
                 ...
                 const router_inputs_t *inputs, router_outputs_t *outputs)
{
    ...
    flit_t demoted_flit;
    flit_t discarded_combine_result;
    ...
    (void)combine_unit_advance(&router->combine, &discarded_combine_result);
    ...
    if (calendar_store_replay(&router->calendar, cycle, &entry) &&
        ...
        if (entry.opcode == CAL_OP_FORWARD) {
            fork_count = multicast_expand(entry.out_port_mask, fork_ports);
            ...
            (void)combine_unit_accept(&router->combine,
                                      (calendar_opcode_t)entry.opcode,
                                      &selected_flit);
        }
    } else {
        if (calendar_store_replay(&router->calendar, cycle, &entry) &&
            ...
            (void)watchdog_demote_arm(&router->watchdog, cycle,
                                      ...
                                      (uint16_t)cycle, entry.out_port_mask);
        } else {
            for (port_index = 0U; port_index < PORT_COUNT; ++port_index) {
                if (inputs->valid[port_index] &&
                    (inputs->flit[port_index].flit_class == FLIT_CLASS_CALENDAR)) {
