...
**几何：** 8×6，48 节点；6 行环×8 + 8 列环×6 = **192 条有向弧**（96 无向，与折叠 torus 链路集逐条相同，金属 96/82 = 1.17× mesh）；H=7，V=9；t_turn=1；RAMP_BW=2。挂接方式推导完之后这就是一个**无缓冲折叠 2D torus**：每个节点既是行环站也是列环站，环内零缓冲、零转环驻留，两维各有双向车道，折叠布线让最长单线只有 2 个核间距
**挂接方式（本轮新增、已推导）：** 每个 AI core 花掉它的 **2 个环口 = 1 个行环口 + 1 个列环口**；两个维度都用**双向全环 + 折叠布线**（最长单线 2 个核间距，没有长绕回线）；**桥与核同址、复用这两个口**，48 个桥的额外环上抽头为 0。这不是假设而是从「≤2 口 + 连通性 + 割容量 + 线长」四条结构约束里筛出来的，九个候选方案的对照见 `results/ring_attach_8x6.json` 与报告 §1
...
**数据：** `results/ring_collectives_8x6.json`、`results/ring_tavg_8x6.json`、`results/ring_robust_8x6.json`、`results/ring_attach_8x6.json`、`results/ring_throughput_8x6.json`（路由无关下界 + 两条腿的 II/带宽利用率）、`results/calendars/ring_*.json`；mesh 参照 `results/multiflit_area_makespan.json`（本轮补齐 R=1/13）
**验证：** `results/verify_ring_collectives_8x6.json`，**90/90 通过**，全部可执行、失败即命名具体量；其中 **6 项记录为「预测被推翻」**而不是悄悄放宽（一项是自查出的记账错误，见第 9 节第 7 条；一项是「半环省金属」这个直觉，见第 9 节第 8 条；一项是 `II_eff` 可以掉到容量界以下，见第 9 节第 10 条）
**报告：** `results/report_ring_collectives_8x6.html` —— 无缓冲环工作的**唯一**报告（中文，围绕「基线 vs 拍图」对比组织，含拓扑与挂接示意图、路由无关下界、makespan 胜负图、II 与带宽利用率、真实拍图甘特图、四个杠杆、T_avg、容错、抗抖动、拍图导出与验证清单）。mesh 侧的 R=1/5/13 深度扫描属于 mesh 既有工作，留在 `results/report_multi_area_makespan.html` §5.5；本报告 §12 只**读取**它的 JSON 作参照，不改那份报告的结论。报告开头先画拓扑示意图，§1 交代挂接方式的推导、§2 给六个集合通信的路由无关下界、§11 给 II 与带宽利用率。
...
17. **挂接方式不用挑，四条结构约束就把它筛成唯一解；而「half ring 更省」是错的。** 九个候选（全环双向 / 全环单车道 / 行半环 / 列半环 / 双半环 / 错位半环 / 单口 / 双口同环，含一个故意不修缝的对照）逐个算连通性、割容量、线长、跳数与 12 个路由无关下界，只有「**双向全环 + 每核 1 行口 1 列口 + 桥与核同址**」同时过掉四道物理门槛。
    - **半跨环按常规的连续二分拆，在「≤2 口」下直接不连通**：列环不改变 x、拆开的行环又跨不过缝，可达对只有 **1104/2256**（左右各 24 核自成孤岛），x 向割容量 **0**。这不是排图能补的。
    - **补缝就得亏带宽**：加缝桥后跨中线从「几条并行环段」塌成「一个存转发 FIFO」，x 向对分 **12 → 6 flit/cy**，alltoall 下界 **48 → 96 拍**（2.00×）；拆列环 1.50×、双向都拆 2.00×。
    - **「半环省金属」被推翻**：闭环上 k 个核就有 k 段，拆环**不改变段数**（96 条无向段、1.17× mesh，九个方案完全一样），省的只是线长（2.0× → 1.85× mesh）；而**错位**半环（唯一不用缝桥还能保住对分的半环）反而更费金属（**2.15×**），且绕过阵列边缘那个环必须有一根跨 **6 个核间距**的长线，折叠压不下去 —— 要么降频要么插寄存器，而寄存器会改掉无缓冲环的时序模型。
    - **单车道那种「半环」在金属恒定下 12 个下界与全环一个不差地打平**，净亏的只有时延（平均 **1.69×**、直径 56 → 95 拍）。**它不是更差的机器，是同样带宽下更慢的机器。**
    - **2 个口正好等于 L1 ramp 的 2 flit/cy**：只用 1 个口进出速率砍半、白扔一半 ramp，端口界从 24 拍翻到 **47 拍**；两个口都押在行环上界不变，但 48 个桥全都得自己在列环上开抽头、平均跳数 3.57 → 5.28。
    这套结构模型是**独立**实现的（`rg_ring_attach.py` 只吃「环怎么连 + 谁开口 + 桥在哪」），却和主流水线报出同一组数：割宽 **12**、需过 **576 flit**、对分界 **48 拍**、直径 **56 拍**。两套独立代码对上，才值得信其中任何一套。

18. **m=1 时这块布局不是带宽问题而是距离问题——六个集合通信的路由无关下界全部由时延绑定。** makespan 界一律是 **55 拍**纯线延迟，容量项最大只有 48 拍；到 m=13 才有三个模式换成容量界（`gather`/`allgather` 撞核端口 = L1 ramp 的 **306 拍**，`alltoall` 撞 cut 的 **624 拍**）。
    所以**「加宽布线 / 加车道」对单 flit 集合通信毫无用处**，该动的是跨度与相位数；而 fan-in 的 306 拍是「root 的 L1 每拍吃 2 个 flit」这条**外设界**，加多少环带宽都省不下来。这些界刻意取最弱的一版（允许中继与本地合并、T0/T1 同界），因此可以同时量两条腿 —— 代价是 broadcast/reduce/allreduce 的容量界弱到 7 拍，那几行的「比下界」倍数衡量的是相位链而不是资源。

19. **连着做的时候排名会换人，而且「带宽利用率」这个词必须拆成占用与有用两个。** 流水稳态（R=13 均摊）下 `alltoall` 基线 **1417.8** vs 拍图 **948.6** 拍/轮（1.50×，II 界 624）；但 `gather` 上两条腿都贴着 L1 ramp 界（1.07× / **1.03×**，m=1），谁也赢不了。
    关键在利用率：`gather/flat` m=13 基线**占用** 25.3%、拍图只占 3.6%，看起来基线把环用得更满 —— 但按最短跳数记账基线的**有用**部分只有 3.6%，和拍图一样。差出来的 **6.96×** 全烧在绕着 root 转圈（偏转 2.66 次/flit）上。**拍图的 hop tax 恒为 1.00。把「弧上有东西在跑」当带宽利用率，会把浪费读成产出。**
...
## 2. 六个集合通信的路由无关下界（先算界，再看谁离它多远）

这一节的界**只依赖拓扑与端口**，不依赖 transport、不依赖能力分层（T0/T1 同界），因此可以同时量基线与拍图。刻意取**最弱**的一版：允许任意中继、允许沿途本地合并，所以「谁都不能低于它」成立，而「贴着它就最优」不成立。

四条界：**cut**（x=1 / y=1 两条直割，容量 12 / 16 flit/cy，取必须跨过的不同 item 数 —— 允许中继，所以只有 alltoall 有乘积项）、**core port**（一个核 2 个环口）、**L1 ramp**（RAMP_BW=2 flit/cy）、**latency**（零竞争最短路时延 + 消息串行化尾巴，转环免费）。前三条是**容量界**，也就是 II 的界；makespan 的界是 `max(容量界, 时延地板)`。本机器上 `2 个环口 = RAMP_BW = 2 flit/cy`，所以 port 与 ramp 两条界数值恒等。

| collective | m | cut | port = ramp | 容量界（=II 界） | 时延地板 | makespan 界 | 绑定 |
|---|---|---|---|---|---|---|---|
| broadcast | 1 | 1 | 1 | 1 | 55 | **55** | latency |
| reduce | 1 | 1 | 1 | 1 | 55 | **55** | latency |
| allreduce | 1 | 1 | 1 | 1 | 55 | **55** | latency |
| gather | 1 | 2 | **24** | 24 | 55 | **55** | latency |
| allgather | 1 | 4 | **24** | 24 | 55 | **55** | latency |
| alltoall | 1 | **48** | 24 | 48 | 55 | **55** | latency |
| broadcast | 13 | 2 | 7 | 7 | 67 | **67** | latency |
| reduce | 13 | 2 | 7 | 7 | 67 | **67** | latency |
| allreduce | 13 | 2 | 7 | 7 | 67 | **67** | latency |
| gather | 13 | 26 | **306** | 306 | 67 | **306** | port |
| allgather | 13 | 46 | **306** | 306 | 67 | **306** | port |
| alltoall | 13 | **624** | 306 | 624 | 67 | **624** | cut |

三条结论：

1. **m=1 时六个集合通信全部是距离问题，不是带宽问题。** makespan 界一律是 **55 拍**的纯线延迟（root 27 到最远节点），而容量项最大只有 48 拍。**「加宽布线 / 加多车道」对单 flit 集合通信毫无用处**，该动的是跨度与相位数 —— 折叠全环把跨度做到最小，正是第 1 条挂接结论的价值所在。
2. **m=13 才有三个模式换成容量界**：`gather` / `allgather` 撞核端口 = L1 ramp（306 拍），`alltoall` 撞 cut（624 拍）。fan-in 的 306 拍是「root 的 L1 每拍只吃 2 个 flit、要吃 47×13 = 611 个」，这是**外设界**而不是网络界，加环带宽一分钱都省不下来。
3. **broadcast / reduce / allreduce 的容量界弱到个位数（7 拍），所以那几行的「比下界」倍数不能当排图质量读**，它衡量的是相位链而不是资源。这三行要评价排图质量必须回到拍图自己模型下那条更紧的界（第 5 节末与报告 §11 后半）。

实测离界多远：m=1 最优拍图 / 结构地板 = broadcast **1.07×**、reduce 1.07×、gather 1.07×、allgather 1.54×、alltoall 1.56×、allreduce **2.04×**（最松的是 allreduce，它要串起最多相位，而地板只按最远那条直线算）。

## 3. Part 1：paper 机制的六个集合通信
...
**下表「下界」那一列是拍图机器模型下的界，比第 2 节的结构界更紧**（结构界 m=1 一律 55 拍；拍图模型再加上每相位 `+RAMP=2`、过桥 `t_turn=1`、抽取点整段独占，于是 broadcast 抬到 59、allreduce 抬到 112）。两套界都合法，量的是两台机器：结构界谁都不能违反，模型界只能用来量它自己那条腿。

**下界那一列算在拍图的机器模型上，不能拿去除 `ring_base`。** 两条腿跑的是两台机器，差别恰好三处：环站出口 1 flit/拍 vs 节点按 `RAMP_BW=2` 排空、拍图每相位多收 `+RAMP=2`、拍图过桥转环收 `t_turn=1` 而 sim 免费。用拍图的界去量基线，40 行里有 **18 行**会出现 <1 的比值（最狠 gather/dim_2phase m=13 的 **0.73×**），下表 `allreduce`/`gather`/`reduce` 那几个 0.89~0.95× 就是这么来的——那是记账口径串了，不是基线跑赢了物理。在基线自己模型下重建的界（`bounds_base`）**40 行全部成立，旋转 10 行精确 1.000×**；口径三项证据与并排表见报告 §4 末与验证第 31–36 项。
...
## 4. Part 1b：allgather 的 T_avg（R = 1 / 5 / 13）
...
## 5. 吞吐（II）与带宽利用率：两条腿同一条公式

单发 makespan 和「连着做能有多快」是两个问题，第 3 节答前者，这一节答后者。两条腿现在用**同一条利用率公式**：`rg_ring_base` 加了逐弧占用计数（每跳记 σ 拍，偏转的每一次穿越都照记），和拍图对 footprint 求和是同一个量。

均摊值取 `per_round = T_R / R`（R=13），**不是** `II_eff`：后者是插值参数，有限 R 下可以低于容量界（见第 9 节第 10 条）。利用率同时报两个数：**占用** = 弧上有东西在跑的比例，**有用** = 每个 flit 只按最短跳数记账；两者之比就是 **hop tax**。

| collective | m | II 下界 | 基线 拍/轮 | 拍图 拍/轮 | base/cal | 基线 占用 / 有用 | 基线 hop tax | 拍图 占用 |
|---|---|---|---|---|---|---|---|---|
| gather | 1 | 24 | 25.8 | **24.7** | 1.04× | 18.1% / 3.4% | **5.31×** | 3.5% |
| gather | 13 | 306 | 312.7 | 313.5 | 1.00× | 25.3% / 3.6% | **6.96×** | 3.6% |
| alltoall | 1 | 48 | 85.8 | **60.5** | 1.42× | 49.2% / 48.9% | 1.01× | 69.5% |
| alltoall | 13 | 624 | 1417.8 | **948.6** | 1.50× | 50.3% / 38.5% | 1.31× | 57.6% |
| allgather | 1 | 24 | 75.8 | **51.9**（T1） | 1.46× | 29.0% / 29.0% | 1.00× | 22.7% |
| allgather | 13 | 306 | 643.5 | 650.8 | 0.99× | 23.7% / 23.7% | 1.00× | 23.5% |
| allreduce | 13 | 7 | **159.1** | 173.5 | 0.92× | 14.8% / 14.8% | 1.00× | 13.5% |
| broadcast | 13 | 7 | **138.5** | 139.3（T0）/ **55.3**（T1） | 0.99× | 4.6% / 4.6% | 1.00× | 5.8%（T1） |

四条必须一起读：

1. **fan-in 上两条腿都到界，谁也赢不了。** `gather` 的 II 下界就是「root 的 L1 每拍吃 RAMP_BW=2 个 flit」：m=1 界 24 拍、基线 25.8（1.07×）、拍图 24.7（**1.03×**）；m=13 界 306、两条腿 312.7 / 313.5（1.02×）。**这个模式已经没有排图空间了，要提速只能加 root 的 ramp 宽度。**
2. **但基线是用 5.3~7.0 倍的带宽换来这个同样的吞吐的。** m=13 `gather` 上基线占用 25.3%、有用只有 3.6%（偏转 **2.66 次/flit**，绕着 root 转圈）；拍图 hop tax 恒为 **1.00**（一跳都没绕）。**把「弧上有东西在跑」当带宽利用率，会把浪费读成产出** —— 所以上表把占用与有用分开列。
3. **带宽受限的模式上拍图赢，而且赢的幅度与单发接近。** `alltoall` m=13 每轮均摊 1417.8 vs **948.6**（1.50×，界 624，拍图到 1.52×）；m=1 是 1.42×。这里绑定的是 **cut**（x=1 割 12 flit/cy、必须过 624 flit），偏转帮不上忙，只会多占弧。
4. **深流水下排名会换人。** `allgather` m=13 单发拍图赢 1.20×，但均摊后两条腿的最优算法都换成 `ring_rotate`、`II_eff` 都是 **611 拍 = 每轮最忙弧的负载**，均摊值 643.5 vs 650.8（**0.99×**，等于打平）；`allreduce`/`broadcast` m=13 均摊基线还略快（0.92× / 0.99×），因为这两个的容量界弱到个位数，绑定的是相位链而不是资源，而基线不收 `+RAMP` 与 `t_turn`（第 10 节第 1 条的口径差）。**T1 的多播在这里才是真增量：broadcast 均摊 139.3 → 55.3 拍/轮（2.52×）。**


## 6. Part 2：四个杠杆与它们各自的可 falsify 预测
...
8. **「half ring 省金属」是错的，而且错在两处。** 拆环不改变段数（96 条无向段在九个方案上完全一样），只改变线长；而**错位**半环 —— 唯一既保住对分又不用缝桥的半环 —— 反而比全环更费金属（2.15× vs 2.0× mesh），还要一根跨 6 个核间距的长线（全环折叠后最长只有 2）。另一半意外是：**单车道环在金属恒定下带宽与全环完全打平**（12 个下界一个不差），所以它的问题只是时延，不是带宽。
9. **基线那些「带宽利用率更高」的格子是浪费被读成了产出。** `gather/flat` m=13 基线占用 25.3%、拍图只占 3.6%，直觉上基线赢；按最短跳数记账后基线的有用部分只有 3.6%，多出来的 **6.96×** 是偏转绕圈（2.66 次/flit）。计划里「利用率」只有一个数，实际必须报两个，否则符号是反的。
10. **`II_eff` 可以掉到容量界以下，它不是可以拿去校验下界的量。** `II_eff=(T_R−T1)/(R−1)` 是让 `T_avg=(T1+T_R)/2` 成立的插值参数：第一轮顺手做掉一部分会被摊掉的工作，于是 `gather/flat` 在 R=5 时 `II_eff=17.5` 拍而容量界是 24 拍（**0.73×**），R=13 收窄到 0.91×。改用均摊值 `T_R/R` 之后恒 ≥ 界，所以第 5 节与报告 §11 报的是均摊值。**违反下界的不是机器，是那个指标的定义。**
11. **1 端口的环在深流水下打不过 mesh**（R=13 输 1.19×），尽管它在单发时延上赢 0.885×。计划隐含假设环的优势会随 R 保持；实际是**绑定资源从跨度换成了环站端口**。同时 mesh 的最优方案自己也在 R=13 换人（axis+CCW → Hamilton bi-tree，II_eff 从 42.25 掉到 1.0），所以两边的排序都不能只在一个 R 上量。
...
7. **基线的 II 与利用率是「假设重排序缓冲无限大」下的读数。** 流水化后目的端重组缓冲峰值到 **611 flit**（`allgather` 类流量），而模型给它配的是 64 flit；仿真器只统计溢出、不丢包、不反压，所以第 5 节里基线那几列偏乐观。要给真实值必须把重组缓冲做成硬约束并让溢出反压回源端，本轮未做。
8. **第 2 节的结构界是刻意取弱的一版。** 允许任意中继与本地合并、T0/T1 同界，所以它的作用是「谁都不能低于它」，不是「谁贴着它就最优」。broadcast/reduce/allreduce 的容量界弱到 7 拍，那几行的「比下界」倍数不能当排图质量读。更紧的界要按算法逐个构造，本轮只对拍图模型做了（第 3 节那张表与报告 §11 后半）。
9. **环 vs mesh 的对照只在「各自设计空间内取最优」这个口径下成立。** mesh 侧扫 W/E/B 三个设计变量（378 点），环侧只扫环站端口数，两边的设计空间不同构，所以表里同时给 1 端口和 2 端口两行而不是一个总结数。加 R=13 后 mesh 扫描约 **900 CPU-分钟**（已改多进程，5 进程约 2.5 小时）；并行版与串行版在抽样点上逐位相同，且复跑出的 axis+CCW T1=96 / T₅=265 与既有 `report_multi_area_makespan.html` 一致。
...
python3 verify_ring_collectives_8x6.py         # 90 项断言（#31–36 基线 vs 其自身模型的界；#37–50 结构下界 / II / 利用率；#51–64 挂接方式）
...
新增代码：`utils/rg_ring_attach.py`（挂接方式设计空间，独立于主流水线）、`utils/dse_ring_throughput_8x6.py`（路由无关下界 + 两条腿的 II 与带宽利用率）、`utils/rg_ring_collectives.py`、`utils/rg_ring_calendar.py`、`utils/dse_ring_collectives_8x6.py`、`utils/dse_ring_tavg_8x6.py`、`utils/dse_ring_robust_8x6.py`、`utils/export_ring_calendars.py`、`utils/verify_ring_collectives_8x6.py`、`utils/gen_ring_collectives_report.py`；扩展：`utils/rg_ring_topo.py`（多播 footprint + `verify_dr` 多播分支）、`utils/rg_ring_base.py`（逐弧占用计数，使基线与拍图用同一条利用率公式）、`utils/dse_ring_collectives_8x6.py`（向上聚合 `link_busy`）、`utils/dse_multiflit_area_makespan.py`（R 列表 + 多进程）。
