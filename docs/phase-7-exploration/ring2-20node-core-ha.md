...
其中 `delay(π)` 是路径上各边 `λ(u,v)` 之和（不再是跳数 × 统一 2 拍）。
...
| 每核 outstanding 读 | 100 | 100 | 100 | 100 | 100 |
| 有 credit 时 RR 上环 | 有 | 有 | — | 有（读请求受 100/核 outstanding 卡） | 有 |
...
|---|---|---|---|
| S0 | 111 | 195.3 | 691.7 |
| S1 | 123.7（AIMD 配置均值；最好 115） | 184.7 | 741.7 |
| S2 iSLIP I=2 | **91** | 153.3 | 553.0 |
| S3 push-on-pull | 111 | 195.3 | 691.7 |
| S4 kind-aware leave | 114 | 192.7 | 699.0 |
| S5 leave-slot lock | 86 | 162.0 | 600.7 |
| S6 oldest dest clash | 86 | 164.3 | 585.3 |
| S7 hop bounce | 81 | 148.7 | 575.7 |
| S8 late plane | 78 | 139.0 | 547.7 |
| S9 late dir | 76 | 143.0 | 539.3 |
| S10 resp late dir | 80 | 142.3 | 527.0 |
| S11 hop hold | 74 | 141.3 | 509.7 |
| S12 hop islip | 71 | **128.3** | 511.0 |
| S13 hop short | 71 | 138.3 | 510.3 |
| S14 HA sib plane | **70** | 137.0 | **507.3** |
| 解析下界 | 47 | 95 | 376 |

**Pareto 图上 S2 有 109 个点，S0–S14 各 1 个。** 同面积 0.0458 上 S14（mk 70）支配 S12/S13（71）、S11（74）、S9（76）、S8（78）、S10（80）、S7（81）和 S5/S6（86）。不是画重了：参考方案各自只有一种硬件结构，而 request-grant 的仲裁器是可设计对象，每组旋钮取值对应一块不同的、都可实现的电路，面积和调度延迟都不同，必须单独评估。旋钮空间 = 算法 9 种（`islip, pim, rr_oldest, lqf, ocf, bvn, greedy_ff, wavefront, batched_bcfs`）× 迭代轮数（islip/pim 取 1,2,4，其余仅 1）→ 13 种组合，× 冲突域 2（`arc` / `whole_ring`）× 占用表示 2（`interval` / `free_at`）× 仲裁器 2 = 104，再加一片补充切片（VOQ 粒度、token 仲裁器、带 RTT 流水线）去重后 109。y 轴已把 `t_sched_cycles` 计回。

按边 hop 时延、上环队列 8 深、端口口径与每核 512 outstanding 对齐之后，**S2 在纯数据面上仍最快**（allpairs DES 91，10k 10403）。把 `t_sched_cycles` 计回后，S14（area 0.0458 / mk 70）**支配**同面积的 S11–S13 和所有更慢的 `ring2_ej` 点。**Pareto 前沿现在是两点：S0（0.0444 / 111）、S14（0.0458 / 70）。** S1（0.0449 / 115）和 S4（0.0444 / 114）被 S0 支配。S2 要赢回前沿，必须把调度延迟压下去，或在更大流量上比面积。

S1 默认用温和 AIMD（α=0.15 / β=0.85 / epoch=64 / rate_min=0.30）。allpairs 均值 123.7、最好 115，**不再**压过 S0 的 111；K=20 仍赢（184.7 vs 195.3）。流量再大开始落后：K=100 是 741.7 vs 691.7，10k 是 18616 vs 15039（1.24×）。教科书组合（0.05 / 0.5 / 32 / 0.05）会把 10k 打到 3×，那是地板太低，不是 AIMD 本身不能用。
...
S4 用 leave 口的种类优先级：allpairs 114（2.43× bound），K=20 略赢 S0（192.7 vs 195.3），K=100 持平偏慢，10k 反而慢到 15207。它解的是「谁先占用共享 leave 口」，不是段带宽，也不是 S2 那种 hop 预约。

S5 预约 dest leave 时隙：allpairs 86（1.83× bound），K=20 / K=100 / 10k 都赢 S0（162 / 601 / 13296），偏转为 0。仍落后 S2 的数据面（91 / 153 / 10403），但面积只比 S0 多一张 64 拍窗口。

S6 把同拍 dest 冲突从节点号改成 oldest：allpairs 仍 86；K=20 162→164.3；K=100 600.7→585.3；10k 13296→13207（1.48× bound），p99 3440→3160。面积与 S5 相同。

S7 在第一跳忙时改绑 plane：allpairs 86→81（1.72× bound）；K=20 164.3→148.7；K=100 585.3→575.7；10k 13207→12880（1.44× bound）。

S8 注入时现场选 hop+dest 都空的 plane：allpairs 81→78（1.66× bound）；K=20 **139.0**；K=100 575.7→547.7；10k 12880→**12071**（1.35× bound）。面积与 S5–S7 相同。

S9 第一跳仍忙则改走另一环方向（≤+2 hop）：allpairs **76**；K=100 **539.3**；10k **11844**（1.32× bound）。

S10 只对响应做这次改向：allpairs 80（输给 S9 的 76）；K=20 142.3；K=100 527.0；10k 11967（略慢于 S9）。

S11 同拍第一跳只留最老响应：allpairs **74**；K=20 141.3；K=100 **509.7**；10k **11515**。

S12 dest-then-hop request-grant：allpairs **71**；K=20 **128.3**；K=100 511.0；10k **11423**。

S13 hop grant 优先短路径：allpairs 71；K=20 138.3；K=100 510.3；10k 11436（略慢于 S12）。

S14 HA sibling plane yield：allpairs **70**；K=20 137.0；K=100 **507.3**；10k **11147**。Pareto 前沿改成 S0 / **S14**。
...
Workload 固定：`uniform` K=2500、R=4、seed=0，`plane_sel=least_occupied`。每个 core 收 **10000** 个响应 flit（10 core × 2500 txn × 4 flit）。十五方案吃同一批事务，同一条数据面（hop 时延按边 1–4 拍、上环队列 8 深、下环 4+1、每核 outstanding 512）。S1 用温和 AIMD：α=0.15、β=0.85、epoch=64、rate_min=0.30。

| 方案 | makespan | 上环成功 | 上环失败 | 偏转 | 上环队列峰值 | 下环队列峰值 | 响应时延 p50 / p99 | outstanding 峰值 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| S0 RR | 15039 | 100000 | 192305 | 11823 | 8 | 1 | 2755 / 3686 | **512** |
| S1 AIMD | 18616 | 100000 | 110774 | 9083 | 8 | 1 | 22 / 187 | 56 |
| S2 iSLIP I=2 | **10403** | 100000 | 0 | 0 | 见下 | 0 | — | **512** |
| S3 push-on-pull | 15039 | 100000 | 192305 | 11823 | 8 | 1 | 2755 / 3686 | **512** |
| S4 kind-aware leave | 15207 | 100000 | 192363 | 11226 | 8 | 1 | 2860 / 3769 | **512** |
| S5 leave-slot lock | 13296 | 100000 | 96775 | **0** | 8 | 1 | 2395 / 3440 | **512** |
| S6 oldest dest clash | 13207 | 100000 | 97428 | **0** | 8 | 1 | 2477 / 3160 | **512** |
| S7 hop bounce | 12880 | 100000 | 73248 | **0** | 8 | 1 | 1850 / 4269 | **512** |
| S8 late plane | 12071 | 100000 | 71012 | **0** | 8 | 1 | 1528 / 4691 | **512** |
| S9 late dir | 11844 | 100000 | 64738 | **0** | 8 | 1 | 1460 / 4543 | **512** |
| S10 resp late dir | 11967 | 100000 | 64854 | **0** | 8 | 1 | 1479 / 4431 | **512** |
| S11 hop hold | 11515 | 100000 | 54587 | **0** | 8 | 1 | 2212 / 2606 | **512** |
| S12 hop islip | 11423 | 100000 | 590 | **0** | 8 | 1 | 2210 / 2533 | **512** |
| S13 hop short | 11436 | 100000 | 781 | **0** | 8 | 1 | 2157 / 2780 | **512** |
| S14 HA sib plane | **11147** | 100000 | 782 | **0** | 8 | 1 | **2145** / **2491** | **512** |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0 | 10000 (5008 / 4992) | 18714 | 10118 | 0 | 18714 | 18593 | 9297 | 9455 | 7090 | 6721 |
| 2 | 10000 (4888 / 5112) | 20669 | 10886 | 0 | 20669 | 19303 | 10302 | 10527 | 7989 | 7480 |
| 4 | 10000 (4920 / 5080) | 18874 | 11960 | 0 | 18874 | 19090 | 10042 | 10134 | 7370 | 7400 |
| 6 | 10000 (4924 / 5076) | 19113 | 11576 | 0 | 19113 | 19355 | 9669 | 9541 | 7302 | 7120 |
| 8 | 10000 (5124 / 4876) | 18209 | 12008 | 0 | 18209 | 19212 | 9517 | 9656 | 7097 | 6632 |
| 10 | 10000 (4828 / 5172) | 18958 | 11970 | 0 | 18958 | 18738 | 9931 | 9695 | 7283 | 7309 |
| 12 | 10000 (5056 / 4944) | 20052 | 10945 | 0 | 20052 | 19417 | 9437 | 9912 | 7261 | 7439 |
| 14 | 10000 (4912 / 5088) | 19074 | 10631 | 0 | 19074 | 18831 | 9450 | 9472 | 7043 | 6716 |
| 16 | 10000 (5032 / 4968) | 19434 | 10262 | 0 | 19434 | 19501 | 9470 | 9608 | 7766 | 7304 |
| 18 | 10000 (5044 / 4956) | 19208 | 10418 | 0 | 19208 | 20323 | 9660 | 9428 | 7047 | 6891 |
| **合计** | **100000 (49736 / 50264)** | **192305** | **110774** | **0** | **192305** | **192363** | **96775** | **97428** | **73248** | **71012** |

**S2 最快（10403 vs S0 15039，快 31%）。** 失败为 0 不是因为没有 I-tag / E-tag，而是 grant 只在 hop 已空时发出，反应式标签在本闭集中突发里用不上。分代预约把每核 in-flight 压在 512 附近。
...
S4 的 10k 均值曲线贴着 S0，但 makespan 更长（15207 vs 15039，1.01×），上环失败略多（192363 vs 192305）。leave 种类优先级在段带宽饱和时帮不上忙，还会打乱 RR 对两个方向的公平。

S5 把偏转打到 0，响应上环失败从 192305 收到 96775，makespan 15039→13296（1.49× bound）。p50 2755→2395。没有消灭 hop 忙，所以仍落后 S2 的 10403。

S6 同预约表、同拍 dest 冲突留最老 flit：makespan 13207（1.48× bound），p99 3440→3160。上环失败略升到 97428——赢的是尾核公平，不是 hop 空闲。

S7 第一跳忙就换 plane：makespan 12880（1.44× bound），响应上环失败 97428→73248，p50 2477→1850。p99 3160→4269。

S8 注入时现场选 plane：makespan **12071**（1.35× bound），上环失败 73248→71012，p50 1850→1528。p99 4269→4691。

S9 第一跳仍忙则改向：makespan **11844**（1.32× bound），上环失败 71012→64738。

S10 只对响应改向：makespan 11967（略慢于 S9），上环失败 64738→64854。

S11 同拍第一跳只留最老响应：makespan 11515（1.29× bound），p99 4543→2606。仍落后 S2 的 10403。

S12 dest-then-hop request-grant：makespan **11423**（1.28× bound）。上环失败从 54587 收到 590，多半是 hold 后不再尝试上环的重分类，不是 hop 争用消失。仍落后 S2 的 10403。

S13 hop grant 优先短路径：makespan 11436（略慢于 S12）。上环失败 590→781。仍落后 S2 的 10403。

S14 HA sibling plane yield：makespan **11147**（1.25× bound）。上环失败 781→782。p50 2145，p99 2491。仍落后 S2 的 10403，是分布式最快。

S0 / S1 / S4 的上环队列都是 8 深且都跑满（峰值 8，S0 有 519713 次 admission stall，S4 有 518676），说明反压真的在起作用。**下环队列峰值只有 1**：4 深的 eject FIFO 在这个负载下根本用不上，S0 的 11823 次偏转来自「每 (node, plane) 每拍只有 1 个 leave 端口，两个方向同拍到达必有一个被挤掉」，不是队列满。E-tag 也因此几乎不触发。真正的限制是 leave 端口数，不是队列深度。

S2 的上环队列占用没法直接比：调度器给出的是刚性 t0，源端提前知道自己什么时候上环，所以只需在 t0 前把 flit 挪进队列，fabric 里的队列需求约为 1。但「已生成、尚未上环」的量峰值达 **273**（`max_src_wait`），这些 flit 停在 PE 侧 backlog 里——S2 把排队从 fabric 推到了源端。
